...
public class E3_FileTypeChange {
...
//        String endString = "\" /><audio>";
            ...
            if ((srcStr != null && srcStr.trim().isEmpty())  || dstStr == null) {
                System.out.println("Src is  empty ");
                System.out.println("参数格式提示:   \"srcz#\"yyyy\"dstz#\"xxxx\"");
            ...
            if(fileTypeStrList.size() > 0){
                ...
                // initDirFileSet(curDirFile);   //  包含所有子目录
                initSingleDirFileSet(curDirFile);  // 不包含子目录
                initSimpleFileSet();
                ...
                for ( File targetFile: allDstFileSet) {
                  //  RenameFileOperation(targetFile);  // 改名的操作
                    // operaFile(targetFile);        // 复制文件的操作
                    RenameFileTypeOperation(targetFile);   // 更改文件类型的操作
                }
      ...
      //  System.out.println("X2 输入参数有误  参数格式提示:  filez#\"txt_md\"srcz#\"yyyy\"dstz#\"xxxx\"");
    ...
    static void toGetTargetFileSet( ArrayList<String> mFileTypeList , Set<File> mSetFile) {
        ...
        for (File itemFile: mSetFile) {
            ...
            if(fileName.lastIndexOf(".") > 0 ){
                ...
                for (int i = 0; i < mFileTypeList.size(); i++) {  //  只有符合 包含关系的 才加入 目标文件集合
                    if("*".equals(mFileTypeList.get(i)) && suffix.contains(staticSrcStr.toLowerCase())){
...
//                    }
    ...
    static void initSingleDirFileSet( File  shellDirFile) {
        if (shellDirFile != null && shellDirFile.exists() && shellDirFile.isDirectory()) {
            ...
            allDirFileSet.add(shellDirFile);
        }
    }
    ...
    public  static String getFileTypeNoPointOri(String fileName){
        String name = "";
        if(fileName.contains(".")){
            name = fileName.substring(fileName.lastIndexOf(".") + 1 ).trim();
        }else{
            name = "";
        }
        return name.trim();
    ...
    static void  RenameFileTypeOperation(File curFile){
        System.out.println("RenameFileTypeOperation   curFile="+ curFile.getAbsolutePath());
        String curFileName = getFileNameNoPointOrigin(curFile.getName());
        if(!curFile.getName().contains(".")){
            System.out.println("当前文件不包含后缀:  File:"+ curFile.getAbsolutePath());
            return;
        }
        String suffix = getFileTypeNoPointOri(curFile.getName());

        String newType = new String(suffix);
        ...
        if(tempGroupStrList != null && tempGroupStrList.size() > 0){
            ...
            for (int i = 0; i < tempGroupStrList.size(); i++) {
                ...
                newType = newType.replaceAll(curTemplateStr.trim(),dstNameOri).trim();
        ...
        if( newType != null && !"".equals(newType) && !newType.equals(suffix) ){  // 完成替换

            tryReName(curFile,curFileName+"."+newType);
        }else{
    ...
    static void RenameFileOperation(File curFile){
        ...
        String srcName = staticSrcStr.toLowerCase().trim();
        String dstName = staticDstStr.toLowerCase().trim();
        String newFileNameNoPoint = "";
// 文件名的小写   都转为小写
        if(curFileName.toLowerCase().contains(srcName.toLowerCase())){

            newFileNameNoPoint = curFileName.replaceAll(srcName,dstName);  // 替换名称

        }

        if(!"".equals(newFileNameNoPoint)){  // 完成替换
            // newFileName  现在是 小写 的名称
            // 原有名称  D1_Jpg_xxx.jpg   只想更改 成D1_Gif_xxx.jpg
// 原有名称  D1_Jpg_xxx.jpg   改 成d1_gif_xxx.jpg

            // d1_jpg_xxx
            String newFileName = newFileNameNoPoint.trim() + filetypeWithPoint.toLowerCase().trim();  // d1_gif_xxx
            String curFilenameOrigin = getFileNameNoPointOrigin(curFile.getName()); //  D1_Jpg_xxx
            String oriSrcName = staticSrcStr.toLowerCase().trim();  //  原有的  需要替换的资源的小写
            String oriFileNameLower = curFilenameOrigin.toLowerCase();
            int srcLength = oriSrcName.length();
            int beginPosition = oriFileNameLower.indexOf(oriSrcName);

            String pre = curFilenameOrigin.substring(0,beginPosition);
            String end  = curFilenameOrigin.substring(beginPosition+srcLength);
            String middle = staticDstStr;
            String newName = pre + middle + end + getFileTypeWithPoint(curFile.getName());
            // src 大小写不敏感   dst 大小写不敏感
            // 获取到它的索引位置  然后又知道 需要替换的名称的长度  就能对原有字符串进行截取
            // System.out.println("beginPosition = "+ beginPosition + "  srcLength ="+ srcLength +"   pre =" + pre + "     middle="+ middle +  "   end ="+ end + "    type ="+ getFileTypeWithPoint(curFile.getName()));
