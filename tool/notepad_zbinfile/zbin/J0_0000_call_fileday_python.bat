...
if "!input_index_1!"=="" (
echo 当前输入文件参数[ %input_index_1%  ] 为空  , 请必须输入一个文件!
GOTO:EOF
)

call :isfileexist_func_1x1 %input_index_1%
if "!isfileexist_return_1!"=="false" (
echo 当前输入文件参数[ %input_index_1%  ] 不是一个文件 , 请检查这个文件!
GOTO:EOF
)
...
rem  current_index == 20210113 
rem  判断这个日期是否在  对应的文件中 

set current_index=!temp_content:~4,8!

rem ECHO  === input_index_1:!input_index_1!      index:!current_index!   input_index_1:%input_index_1% 

call :isfilecontainoneline_func_2x1  %input_index_1%  !current_index!
echo isfilecontainoneline_return_1=!isfilecontainoneline_return_1!
if "!isfilecontainoneline_return_1!"=="true" (
...
:isfilecontainoneline_func_2x1
rem ======================================== isfilecontainoneline_func_2x1
rem desc: 读取指定全路径文件的每行内容
rem sample: call :isfilecontainoneline_func_2x1  %zbin%\A.bat  20200101
rem sample_out: 读取了 C:\Users\xxxx\Desktop\zbin\A.bat 的每行内容
::SETLOCAL
echo ______________Method_In isfilecontainoneline_func_2x1
echo isfilecontainoneline_func_2x1 input_1_param == %1
echo isfilecontainoneline_func_2x1 input_2_param == %2
set  isfilecontainoneline_return_1=false
set /a filecontainoneline_rownum=0
if exist %1 (
echo isfilecontainoneline_func_2x1 Success Read __: File not exist for __: isfilecontainoneline_func_2x1 input_1_param == %1
  for /f "delims=" %%l  in (%1) do ( 
       set /a filecontainoneline_rownum+=1 
       set s=%%l
	   echo line[!filecontainoneline_rownum!] = %%l
	   if "%2"=="%%l" (
       set  isfilecontainoneline_return_1=true
	   echo [isfilecontainoneline_func_2x1 EndPrintCode]    output=[!isfilecontainoneline_return_1!] param1=[%1]   param2=[%2]   
       echo ______________Method_Out isfilecontainoneline_func_2x1
	   ::ENDLOCAL
	   goto:eof
        ) 
       )
