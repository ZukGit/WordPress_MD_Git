...
public class F6_ZChar {
    ...
    static final String[] CharUnicodeEncodes = new String[] { "Big5" ,"windows-1252","UTF-8","GBK","GB2312","GB18030","ISO-8859-1","ISO-8859-15"  };
...
//            String mShowWithmCharsetA;  //  A-- 可能的正确编码的方式
            ...
            if(curOS_TYPE == OS_TYPE.Windows){
                commandNotead = "cmd.exe /c start   Notepad++.exe " + TEXT_INPUT_FILE;
            }else if(curOS_TYPE == OS_TYPE.Linux){
                commandNotead  = " gedit " + TEXT_INPUT_FILE;
            ...
            File curFile = new File(firstParam);
            if(!curFile.exists()){
                ...
                curFilePath = firstParam;


                //  只有一个参数的情况
                if(mKeyWordName.size() ==1){

                    String targetChar = detect(curFilePath);

                    System.out.println("当前文件[ "+curFilePath+" ] 的编码方式(原本的正确的编码方式):"+ targetChar);
                    ArrayList<String> lumaList = new ArrayList<String>();
                    ReadLuanMaFromFileContent(curFilePath,lumaList);
                    ArrayList<String> lumaLog = new ArrayList<String>();
                    String tag = "";
                    for (int i = 0; i < lumaList.size(); i++) {
                        lumaLog.add("乱码索引 "+i +":"+ lumaList.get(i)+"  【"+ "】");

                        if( i == 0){
                            tag = tag + lumaList.get(i);
                        }else if(i == lumaList.size() -1 ){
                            tag = tag + lumaList.get(i);
                        }
                    }
                    ...
                    if(!"".equals(tag)){
                        ...
                        checkAllCharSet(tag);
                        LinkedHashMap<String,String> arr2titleMapA = new LinkedHashMap<String,String>();
                        arr2titleMapA.put("mCharsetBName","B编码(可能的错误编码)【"+tag+"】");
                        arr2titleMapA.put("mCharsetAName","A编码(可能的正确编码)");
                        arr2titleMapA.put("mShowWithmCharsetA","A结果形式");
                        TablePrint(allCharSetResultList.toArray(),arr2titleMapA,"文本中读取字符串【"+tag+"】显示字符集情况");
                        OutStrList.addAll(tableListStr);

                        String log1 = "【"+tag+"】的ASCII码 【 "+native2Ascii(tag)+" 】";
                        String log2 = "【"+tag+"】的"+LOCAL_ENCODEING+"(本地Native)格式 【 "+ascii2Native(tag)+" 】";
                        System.out.println(log1);
                        System.out.println(log2);
                        OutStrList.add(log1);
                        OutStrList.add(log2);
                        ...
                        ShowFixedFuncTion(curFilePath,OutStrList);

                    }
                    //   TablePrint()
//                    ArrayPrint(lumaLog,"文件可能的乱码");
                ...
                }else if(mKeyWordName.size() ==3){  // 三个参数的情况
                    String path = curFilePath;
                    String inputChar = mKeyWordName.get(1);
                    String outputChar = mKeyWordName.get(2);
                    String localChar = detect(curFilePath);
                    fixedSelectedCharSet(path, localChar,inputChar,outputChar);
...
//            String commandNotead = "cmd.exe /c start   Notepad++.exe " + TEXT_OUT_FILE;
            String commandNotead = "";
            if(curOS_TYPE == OS_TYPE.Windows){
                commandNotead = "cmd.exe /c start   Notepad++.exe " + TEXT_OUT_FILE;
            }else if(curOS_TYPE == OS_TYPE.Linux){
                commandNotead  = " gedit " + TEXT_OUT_FILE;
            }else if(curOS_TYPE == OS_TYPE.MacOS){
                commandNotead  = " gedit " + TEXT_OUT_FILE;
            }
...
*/
    ...
    public static void fixedSelectedCharSet(String inputPath ,String localCharSet , String inputFileEncode ,String outputFileEncode ){

//        System.out.println("开始执行转换编码格式操作 "+ inputFileEncode +" --> "+ inputFileEncode);
        try {
          ArrayList<String>  content =   ReadAsSelectCharSet(inputPath,localCharSet);
          ArrayList<String> fixedContent = new    ArrayList<String>();

            System.out.println("本地编码A:"+ localCharSet +"   可能错误编码B:"+inputFileEncode + "  可能正确编码C: "+ outputFileEncode);


            System.out.println("原始第一行输出:"+content.get(0));
            for (int i = 0; i < content.size(); i++) {
                String curStrItem = content.get(i);
//                String fixedStr = new String(curStr.getBytes(inputFileEncode),inputFileEncode);
//                fixedStr = new String(fixedStr.getBytes(localCharSet),outputFileEncode );
//                fixedContent.add(fixedStr);

                byte[] byteArr = curStrItem.getBytes(inputFileEncode);
                String curStrFixef = new String(curStrItem.getBytes(inputFileEncode),outputFileEncode);
//                currentCharSet.originSrcStr = new String(srcStr);
                fixedContent.add(curStrFixef);




/*                byte[] byteArr = srcStr.getBytes(currentCharSet.mCharsetBName);    // B 是可能的错误乱码
            ...
            System.out.println("转换第一行输出:"+fixedContent.get(0));




            WriteAsSelectCharSet(inputPath,fixedContent,outputFileEncode);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    public static ArrayList<String> ReadAsSelectCharSet(String inputPath , String inputFileEncode) throws IOException {
//     StringBuilder  mContent = new StringBuilder();
     ArrayList<String> mContent = new  ArrayList<String>();
        BufferedReader bufferedReader = new BufferedReader(
                new InputStreamReader(new FileInputStream(inputPath), inputFileEncode));
...
//            mContent.append(line+"\n");
    ...
    public static void WriteAsSelectCharSet(String inputPath ,  ArrayList<String>  content, String outputFileEncode) throws IOException {

        System.out.println("输出文件编码:"+ outputFileEncode);
        BufferedWriter bufferedWriter = new BufferedWriter(
                new OutputStreamWriter(new FileOutputStream(inputPath), outputFileEncode));
    ...
    public static void ShowFixedFuncTion(String filePath , ArrayList<String>  outStrList) {
...
int index  = 1;
        ...
        for (int i = 0; i < CharUnicodeEncodes.length; i++) {
           String charA = CharUnicodeEncodes[i];
            for (int j = 0; j < CharUnicodeEncodes.length; j++) {
                String charB = CharUnicodeEncodes[j];
                arrLog.add("方法索引 "+index+":  "+ pre.trim() + " " + filePath + "  "+ charA + " "+charB );
                index ++;
            }
        }

        outStrList.add("=========="+"修复方法列表"+"==========" );
        outStrList.addAll(arrLog);

        ArrayPrint(arrLog,"修复方法列表");
    ...
    public static ArrayList<String> ReadLuanMaFromFileContent(String path , ArrayList<String> luanmaList) {
        ...
        if (curFile != null) {
            try {
                ...
                while (oldOneLine != null) {
                    ...
                    String maybeLuanMa = revertAsciiStr(oldOneLine).trim();

                    if("".equals(maybeLuanMa)){
                        continue;
                    }else{
                    //    System.out.println("可能乱码:" + maybeLuanMa);
                    }
                    luanmaList.add(maybeLuanMa);
//                    System.out.println("第"+index+"行读取到的字符串:"+oldOneLine);
    ...
    public static String revertAsciiStr(String src){
        String dst = ""+src;
        dst = clearChinese(dst);
        dst=dst.replace(" ","");
        ...
        dst=dst.replace(".","").trim();
        dst=dst.replace("/","").trim();
        dst=dst.replace("0","").trim();
        dst=dst.replace("1","").trim();
        dst=dst.replace("2","").trim();
        dst=dst.replace("3","").trim();
        dst=dst.replace("4","").trim();
        dst=dst.replace("5","").trim();
        dst=dst.replace("6","").trim();
        dst=dst.replace("7","").trim();
        dst=dst.replace("8","").trim();
        dst=dst.replace("9","").trim();
        dst=dst.replace(":","").trim();
        dst=dst.replace(";","").trim();
        dst=dst.replace("<","").trim();
        dst=dst.replace("=","").trim();
        dst=dst.replace(">","").trim();
        dst=dst.replace("?","").trim();
        dst=dst.replace("A","").trim();
        dst=dst.replace("B","").trim();
        dst=dst.replace("C","").trim();
        dst=dst.replace("D","").trim();
        dst=dst.replace("E","").trim();
        dst=dst.replace("F","").trim();
        dst=dst.replace("G","").trim();
        dst=dst.replace("H","").trim();
        dst=dst.replace("I","").trim();
        dst=dst.replace("J","").trim();
        dst=dst.replace("K","").trim();
        dst=dst.replace("L","").trim();
        dst=dst.replace("M","").trim();
        dst=dst.replace("N","").trim();
        dst=dst.replace("O","").trim();
        dst=dst.replace("P","").trim();
        dst=dst.replace("Q","").trim();
        dst=dst.replace("R","").trim();
        dst=dst.replace("S","").trim();
        dst=dst.replace("T","").trim();
        dst=dst.replace("U","").trim();
        dst=dst.replace("V","").trim();
        dst=dst.replace("W","").trim();
        dst=dst.replace("X","").trim();
        dst=dst.replace("Y","").trim();
        dst=dst.replace("Z","").trim();
        dst=dst.replace("[","").trim();
        dst=dst.replace("/","").trim();
        dst=dst.replace("]","").trim();
        dst=dst.replace("^","").trim();
        dst=dst.replace("_","").trim();
        dst=dst.replace("\n","");
        dst=dst.replace("\t","");
        dst=dst.replace("\b","");
        dst=dst.replace(" ","");
...
//            }
    ...
    public static void ShowTip() { //从字节读取内容
        ...
        System.out.println(" zchar_F6.bat    北京 上海   (有参 非路径)-- 将解析对应的输入字符串并打印编码解码表 ASCII码 Native码");
        System.out.println(" zchar_F6.bat    /xxxxxx/xxxx/daf.txt   (有 1个参数)--  解析当前路径文件保存的编码方式! 并显示转换列表 ");
        System.out.println(" zchar_F6.bat    /xxxxxx/xxxx/daf.txt   UTF-8  GBK  (有 3个参数)--  把当前的文件编码从 UTF-8 转为 GBK");
    }
...
//            }
            ...
            // 对当前的最大长度进行修正
            // 如果大小小于  DEFAULT_TABLE_CONTENT_PADDING_SIZE_MIN  8 , 那么就设置为 8
            //
            int curTableItemNum = row_table_num;  // 每行的 显示项
            int curTableMaxLength = DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX;
...
//                    System.out.println(index+ "未修正 Value = "+ entry.getValue());
                    if(entryA.getValue() < DEFAULT_TABLE_CONTENT_PADDING_SIZE_MIN){
                        fixed_MaxSizeMap.put(entryA.getKey(),DEFAULT_TABLE_CONTENT_PADDING_SIZE_MIN);
                    }else if(entryA.getValue() > DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX){
                        fixed_MaxSizeMap.put(entryA.getKey(),DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX);
                    }else{
            ...
            for (int i = 0; i < showItem.length; i++) {
                ...
                for (int j = 0; j < filedAndSizelist.size(); j++) {
                    ...
                    try {
                        ...
                        while(rawSize > DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX){
                            curTableItem = curTableItem.substring(0,DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX-1);
...
|| (itemChar+"").equals("；")
    ...
    static int DEFAULT_TABLE_CONTENT_PADDING_SIZE_MIN = 8;  // 最小单元
    static int DEFAULT_TABLE_CONTENT_PADDING_SIZE_MAX = 50; // 最大党员
