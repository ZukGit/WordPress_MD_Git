...
def initSystemInfo():
    print(
        "════════════════" + "System Info Begin" + "【sys.platform=" + str(sys.platform) + "】【platform.system()=" + str(
            platform.system()) + "】════════════════")
    ...
    print("CUR_OS_FileSeq:" + str(OS_SEQ))
...
class Rule:
    ...
    def ruleTip(self):
        ...
        return CUR_Bat_Name + "  " + "#_" + str(self.rule_index) + "   ## [索引 " + str(
            self.rule_index) + "]  描述:" + str(simpdescStr)
...
class Snake(object):
    ...
    def is_tail_inside(self):
        self.tmpboard[
            self.tmpsnake[self.tmpsnake_size - 1]] = SnakeConf.FOOD  # 虚拟地将蛇尾变为食物(因为是虚拟的，所以在tmpsnake,tmpboard中进行)
    ...
    def draw_snake(self, screen):
        ...
        for i in range(1, self.snake_size):
            left_pre = (self.snake[i - 1] // SnakeConf.WIDTH) * SnakeConf.LINE_WIDTH + 2 + int(
                Device_Width // 4)  # margin 2
            ...
            if (top_pre == top):
                rect = pygame.Rect(min(left, left_pre), top, SnakeConf.LINE_TRUEWIDTH + SnakeConf.LINE_WIDTH,
                                   SnakeConf.LINE_TRUEWIDTH)
            else:
                rect = pygame.Rect(left, min(top, top_pre), SnakeConf.LINE_TRUEWIDTH,
                                   SnakeConf.LINE_TRUEWIDTH + SnakeConf.LINE_WIDTH)
            if (i == 1):
    ...
    def snake_main(self):
        ...
        while True:
            ...
            boardrect = pygame.Rect(SnakeConf.LINE_WIDTH + int(Device_Width // 4), SnakeConf.LINE_WIDTH,
                                    SnakeConf.SCREEN_X - SnakeConf.LINE_WIDTH * 2,
                                    SnakeConf.SCREEN_Y - SnakeConf.LINE_WIDTH * 2)
            pygame.draw.rect(screen, SnakeConf.WHITE_COLOR, boardrect, 0)

            self.draw_snake(screen)

            rect = pygame.Rect((self.food // SnakeConf.WIDTH) * SnakeConf.LINE_WIDTH + 2 + int(Device_Width // 4),
                               (self.food % SnakeConf.WIDTH) * SnakeConf.LINE_WIDTH + 2, SnakeConf.LINE_TRUEWIDTH,
                               SnakeConf.LINE_TRUEWIDTH)
    ...
    class Rule4_FoodManager:
        ...
        def draw_fruit(self, screen, snake):
            ...
            pygame.draw.rect(screen, Rule4_RED, \
                             (self.fruit_pos[0] * Rule4_USIZE, self.fruit_pos[1] * Rule4_USIZE, Rule4_USIZE,
                              Rule4_USIZE))
...
def rule6_main(yanhua_count):
    ...
    for i in range(yanhua_count):
        ...
        yanhua_list.append(
            Yanhua(300, 300, -20, n=100, color=[color_r_rule6, color_g_rule6, color_b_rule6], v=speed_rule6))
...
if __name__ == '__main__':
    ...
    if CUR_Rule_Index_Int != -1 and CUR_Rule_Index_Int <= len(realTypeRuleList):
        ...
        print("当前索引搜索无效 请检查-打印Tip CUR_Rule_Index=" + CUR_Rule_Index + "  规则总数:" + str(
            len(realTypeRuleList)))
        ShowAllRuleTip()