...
//
public class I7_Bat_SH_Create {


    /*******************修改属性列表 ------Begin *********************/
// 修改0.   全局修改 把 I7 改为当前应用的序号规则序号  当前类名称也需要修改
// 修改1.当前 执行代码的 bat sh 文件名称  最后必须是标识序号
// 修改3.  当前是否有默认的规则   如果有默认的规则那么设置 CUR_TYPE_INDEX为对应index , 没有默认规则那么设置为默认的1
    static String Cur_Bat_Name = "zbat_sh_template_create_I7";
...
//  固定属性列表 ------Begin
    ...
    static File I7_Properties_File = new File(System.getProperties().getProperty("user.home") + File.separator + "Desktop" + File.separator + "zbin" + File.separator + get_Bat_Sh_FlagNumber(Cur_Bat_Name)+".properties");
    static InputStream I7_Properties_InputStream;
    static OutputStream I7_Properties_OutputStream;
    static Properties I7_Properties = new Properties();
...
//  固定属性列表 ------End
    ...
    // ffmpeg  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\I7_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output2.mp4
    // D:\software\ffmpeg\bin
    // D:\software\ffmpeg\bin\ffmpeg.exe  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\I7_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output3.mp4
    ...
    static {
        try {
            if (!I7_Properties_File.exists()) {
                I7_Properties_File.createNewFile();
            }
            I7_Properties_InputStream = new BufferedInputStream(new FileInputStream(I7_Properties_File.getAbsolutePath()));
            I7_Properties.load(I7_Properties_InputStream);
            Iterator<String> it = I7_Properties.stringPropertyNames().iterator();
            while (it.hasNext()) {
                String key = it.next();
                // System.out.println("key:" + key + " value: " + I7_Properties.getProperty(key));
                propKey2ValueList.put(key, I7_Properties.getProperty(key));
            }
            I7_Properties_InputStream.close();
    ...
    static void setProperity() {
        try {
            I7_Properties_OutputStream = new BufferedOutputStream(new FileOutputStream(I7_Properties_File.getAbsolutePath()));
            I7_Properties.store(I7_Properties_OutputStream, "");
            I7_Properties_OutputStream.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    void InitRule(){
        ...
        //   加入类型一一对应的 那些 规则
        CUR_RULE_LIST.add( new No_Jar_Create_1());
        CUR_RULE_LIST.add( new One_Jar_Create_2());
...
//        CUR_RULE_LIST.add( new ClearChineseType_8());
    ...
    class One_Jar_Create_2 extends  Basic_Rule{
        Map<String,Map<File,File>> NoJar_Identify_File_Map; ;
        ArrayList<String> identifyStrList;
        File zbin_I7_Java_Template_File;  //  Desktop/zbin/I7_0jar_Template.java    Java 文件的模板
        File zbin_I7_Bat_Template_File;  // Desktop/zbin/I7_0jar_Bat_Template.bat 的模板文件
        File zbin_I7_Sh_Template_File;  // Desktop/zbin/I7_0jar_Sh_Template.sh  的模板文件
        File winZbin_I7_Sh_Template_File;  //Desktop/zbin/I7_0jar_Sh_mac_lin_zbin_Template.sh
        File winZbin_I7_Bat_Template_File;  //Desktop/zbin/I7_0jar_Bat_win_zbin_Template.bat
        ...
        One_Jar_Create_2(){
            super(2);
            NoJar_Identify_File_Map = new HashMap<String,Map<File,File>>();
            identifyStrList  = new  ArrayList<String>();
            zbin_I7_Java_Template_File = new File(zbinPath+File.separator+"I7_1jar_Template.java");
            zbin_I7_Bat_Template_File = new File(zbinPath+File.separator+"I7_1jar_Bat_Template.bat");
            zbin_I7_Sh_Template_File = new File(zbinPath+File.separator+"I7_1jar_Sh_Template.sh");
            winZbin_I7_Sh_Template_File = new File(zbinPath+File.separator+"I7_1jar_Sh_mac_lin_zbin_Template.sh");
            winZbin_I7_Bat_Template_File = new File(zbinPath+File.separator+"I7_1jar_Bat_win_zbin_Template.bat");

            ZbinJavaFile = zbin_I7_Java_Template_File;

            if(CUR_OS_TYPE == OS_TYPE.Windows){
                WinZbinExeFile = winZbin_I7_Bat_Template_File;
                ZbinExeFile = zbin_I7_Bat_Template_File;
            }else{
                WinZbinExeFile = winZbin_I7_Sh_Template_File;
                ZbinExeFile = zbin_I7_Sh_Template_File;
            }
        ...
        boolean checkParamsOK(File shellDir, String type2Param, ArrayList<String> otherParams) {
            ...
            for (int i = 0; i <otherParams.size() ; i++) {
                Map<File,File> curFileMap = new HashMap<File,File>();
                String paramsItem = otherParams.get(i);
                String one_two_str = paramsItem.substring(0,2);
                if(one_two_str == null ){
                    continue;
                }
                String batIdentifyStr = one_two_str.toUpperCase();
                boolean isIdentifyBatExist =  checkIdentifyExist(batIdentifyStr,zbinPath);
                if(isIdentifyBatExist){
                    System.out.println("当前的第"+i+"个输入参数 "+paramsItem+" 的 标识"+batIdentifyStr+"已存在 无法创建新的该标识对应的 No-Jar 执行文件!");
                    continue;
                }

                if(!isAZ(batIdentifyStr.substring(0,1)) || !isNumeric(batIdentifyStr.substring(1,2))){
                    System.out.println("当前的第"+i+"个输入参数 "+paramsItem+" 的 标识"+batIdentifyStr+"不符合命名规则 无法创建新的该标识对应的 No-Jar 执行文件!");
                    continue;
                }



                String zbinItemFileItemPath = zbinPath+File.separator+batIdentifyStr+BAT_OR_SH_Point;
                String win_zbinFileItemPath = cur_os_zbinPath+File.separator+"z_"+getTimeStamp()+"_"+batIdentifyStr+BAT_OR_SH_Point;
                String zbinItemJavaFilePath =  zbinPath+File.separator+batIdentifyStr+".java";

                File zbinItemFileItem = new File(zbinItemFileItemPath);
                File win_zbinFileItem = new File(win_zbinFileItemPath);
                File zbinItemJavaFile = new File(zbinItemJavaFilePath);

                curFileMap.put(zbinItemJavaFile,ZbinJavaFile);
                curFileMap.put(win_zbinFileItem,WinZbinExeFile);
                curFileMap.put(zbinItemFileItem,ZbinExeFile);
                NoJar_Identify_File_Map.put(batIdentifyStr,curFileMap);
                // 1.    I7.bat  或者   I7.sh
                // 2.    I7.java
                // 3.    win_zbin/stamp_I7.bat 或者  lin_zbin/stamp_I7.bat   mac_zbin/stamp_I7.bat
        ...
        ArrayList<File> applyOperationRule(ArrayList<File> curFileList, HashMap<String, ArrayList<File>> subFileTypeMap, ArrayList<File> curDirList, ArrayList<File> curRealFileList) {
            Map.Entry<String , Map<File,File>> entry;
            Map.Entry<File , File> entryB;

            if(NoJar_Identify_File_Map != null){
                Iterator iterator = NoJar_Identify_File_Map.entrySet().iterator();


                while( iterator.hasNext() ){
                    entry = (Map.Entry<String , Map<File,File>>) iterator.next();
                    String identify = entry.getKey();  //Map的Value
                    Map<File,File> fileMap = entry.getValue();  //Map的Value

                    Iterator iteratorB = fileMap.entrySet().iterator();

                    while(iteratorB.hasNext()){
                        entryB = (Map.Entry<File , File>) iteratorB.next();
                        File targetFile = entryB.getKey();  //Map的Value
                        File originFile = entryB.getValue();  //Map的Value
...
//                itemDesc = batName.trim() + "A1 A2 A3 J2 J3    【默认】 [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3 等 行形式的 没有jar包引用的 zbin win_zbin等 BAT 文件结构";
                itemDesc += "\n"+ batName.trim()+"  "+type+"_"+index + " A1 A2 A3 J2 J3    [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3   等 行形式的 包含一个jar包引用的 zbin win_zbin等 BAT 文件结构";

            }else{
//                itemDesc = batName.trim() + "A1 A2 A3 J2 J3     [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3 等 行形式的 没有jar包引用的 zbin win_zbin等 SH 文件结构";
                itemDesc += batName.trim()+" "+type+"_"+index + "  A1 A2 A3 J2 J3  [索引 "+index+"]  描述:"+"指定生成 如 Z1 A1 A9 等 行形式的 包含一个jar包 引用的 zbin mac_zbin lin_zbin等 SH 文件文件结构";
            }
    ...
    class No_Jar_Create_1 extends  Basic_Rule{
        Map<String,Map<File,File>> NoJar_Identify_File_Map; ;
        ArrayList<String> identifyStrList;
        File zbin_I7_Java_Template_File;  //  Desktop/zbin/I7_0jar_Template.java    Java 文件的模板
        File zbin_I7_Bat_Template_File;  // Desktop/zbin/I7_0jar_Bat_Template.bat 的模板文件
        ...
        No_Jar_Create_1(){
            ...
            NoJar_Identify_File_Map = new HashMap<String,Map<File,File>>();
            identifyStrList  = new  ArrayList<String>();
            zbin_I7_Java_Template_File = new File(zbinPath+File.separator+"I7_0jar_Template.java");
            zbin_I7_Bat_Template_File = new File(zbinPath+File.separator+"I7_0jar_Bat_Template.bat");
            zbin_I7_Sh_Template_File = new File(zbinPath+File.separator+"I7_0jar_Sh_Template.sh");
            winZbin_I7_Sh_Template_File = new File(zbinPath+File.separator+"I7_0jar_Sh_mac_lin_zbin_Template.sh");
            winZbin_I7_Bat_Template_File = new File(zbinPath+File.separator+"I7_0jar_Bat_win_zbin_Template.bat");
        ...
        String ruleTip(String type,int index , String batName,OS_TYPE curType){
            ...
            if(curType == OS_TYPE.Windows){
                itemDesc = batName.trim() + "A1 A2 A3 J2 J3    【默认】 [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3 等 行形式的 没有jar包引用的 zbin win_zbin等 BAT 文件结构";
                itemDesc += "\n"+ batName.trim()+"  "+type+"_"+index + " A1 A2 A3 J2 J3    [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3   等 行形式的 没有jar包引用的 zbin win_zbin等 BAT 文件结构";

            }else{
                itemDesc = batName.trim() + "A1 A2 A3 J2 J3     [索引 "+index+"]  描述:"+" 指定生成 如 A1 A2 A3 J2 J3 等 行形式的 没有jar包引用的 zbin win_zbin等 SH 文件结构";
                itemDesc += batName.trim()+" "+type+"_"+index + "    [索引 "+index+"]  描述:"+"指定生成 如 Z1 A1 A9 等 行形式的 没有jar包引用的 zbin mac_zbin lin_zbin等 SH 文件文件结构";
            }
...
/*
            String itemDesc = "";
           if(CUR_OS_TYPE == OS_TYPE.Windows){
                itemDesc = "zrule_apply_I7.bat  "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }else{
               itemDesc = "zrule_apply_I7 "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }
           */
...
//        System.out.println("══════════════End ExE ");
    ...
    public static void fileCopy(File origin, File target) {
        System.out.println("生成文件 -> "+target.getAbsolutePath() );
    ...
    public static void main(String[] args) {
        ...
        I7_Bat_SH_Create mI7_Object = new I7_Bat_SH_Create();
        mI7_Object.InitRule();


        // 用户没有输入参数
        if (CUR_INPUT_3_ParamStrList.size() == 0) {
            System.out.println("用户输入的参数为空!");
            showTip();
            return;
        }
...
*/
    ...
    // A1  A2 A3 A5  I8  I9  这样的标识
    boolean checkIdentifyExist(String twoCharIndetify , String mDesktopZBinPath){
        boolean flag = false;
        File zbinFile = new File(mDesktopZBinPath);
        if(!zbinFile.exists()){
            return false;
        }
        File[] zbinFileList = zbinFile.listFiles();

        for (int i = 0; i < zbinFileList.length ; i++) {
            File fileItem = zbinFileList[i];
            String fileName = fileItem.getName();
            if(fileName.length() < 2){
                continue;
            }
            String pre2Char = fileName.substring(0,2).toUpperCase();
            if(twoCharIndetify.equals(pre2Char)){
                return true;  //  存在
            }
        }
        return flag;
