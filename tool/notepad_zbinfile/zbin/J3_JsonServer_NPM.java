...
//
public class J3_JsonServer_NPM {


    /*******************修改属性列表 ------Begin *********************/
// 修改0.   全局修改 把 J3 改为当前应用的序号规则序号  当前类名称也需要修改
// 修改1.当前 执行代码的 bat sh 文件名称  最后必须是标识序号
    //修改2. J3_Common_Template  改为当前类名称  A1 Z9
    //修改2.1  J3 改为 对应的 标识符
// 修改3.  当前是否有默认的规则   如果有默认的规则那么设置 CUR_TYPE_INDEX为对应index , 没有默认规则那么设置为默认的1
    static String Cur_Bat_Name = "zjson_server_J3";
...
//固定1  zbin 的 字符串绝对路径
    ...
    static File J3_JSON_File = new File(System.getProperties().getProperty("user.home") + File.separator + "Desktop" + File.separator + "zbin" + File.separator +"J3_Json_Template.json");



    // 固定2 当前执行文件的编号 A1  A2  A3   ... G1   J3   G3 ... Z9
    static File J3_Properties_File = new File(System.getProperties().getProperty("user.home") + File.separator + "Desktop" + File.separator + "zbin" + File.separator + get_Bat_Sh_FlagNumber(Cur_Bat_Name)+".properties");
    static InputStream J3_Properties_InputStream;
    static OutputStream J3_Properties_OutputStream;
    static Properties J3_Properties = new Properties();
...
//  固定属性列表 ------End
    ...
    // ffmpeg  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\J3_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output2.mp4
    // D:\software\ffmpeg\bin
    // D:\software\ffmpeg\bin\ffmpeg.exe  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\J3_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output3.mp4
    ...
    static {
        try {
            if (!J3_Properties_File.exists()) {
                J3_Properties_File.createNewFile();
            }
            J3_Properties_InputStream = new BufferedInputStream(new FileInputStream(J3_Properties_File.getAbsolutePath()));
            J3_Properties.load(J3_Properties_InputStream);
            Iterator<String> it = J3_Properties.stringPropertyNames().iterator();
            while (it.hasNext()) {
                String key = it.next();
                // System.out.println("key:" + key + " value: " + J3_Properties.getProperty(key));
                propKey2ValueList.put(key, J3_Properties.getProperty(key));
            }
            J3_Properties_InputStream.close();
    ...
    static void setProperity() {
        try {
            J3_Properties_OutputStream = new BufferedOutputStream(new FileOutputStream(J3_Properties_File.getAbsolutePath()));
            J3_Properties.store(J3_Properties_OutputStream, "");
            J3_Properties_OutputStream.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    static Map<String,File> shellName_File_Map = new HashMap<String,File>();
    ...
    public static boolean checkNpm() {
       ...
       Set<String> pathExeSet =  shellName_File_Map.keySet();
ArrayList<String> pathExeList = new ArrayList<String>();
        pathExeList.addAll(pathExeSet);
        flag =   pathExeSet.contains("npm.cmd");
    ...
    public static boolean checkJsonServer() {
        boolean flag = false;

        Set<String> pathExeSet =  shellName_File_Map.keySet();
        ArrayList<String> pathExeList = new ArrayList<String>();
        pathExeList.addAll(pathExeSet);
        flag =   pathExeSet.contains("json-server.cmd");
    ...
    public static void Init_PathInfo() {
        ArrayList<String> pathInfoList = new ArrayList<String>();
        if (CUR_OS_TYPE == OS_TYPE.Windows) {
            int order_index = 1;
            int sub_order_index = 1;
            for (int i = 0; i < EnvironmentList.length; i++) {
                String itemPath = EnvironmentList[i];
                File destFile = new File(itemPath);
                if(destFile.exists() && itemPath.contains(":")){
                  //  pathInfoList.add("======Path_Index:"+getPaddingIntString(order_index,4," ",true)+"  "+itemPath.trim());
                    ArrayList<File> subExeList =     getPathExeFileName(destFile);
                    for (int j = 0; j < subExeList.size(); j++) {
                        File exeNameFile = subExeList.get(j);

                        shellName_File_Map.put(exeNameFile.getName(),exeNameFile);
                  //      pathInfoList.add("      Path_Index:"+getPaddingIntString(sub_order_index,4," ",true)+"  Sub_Exe["+getPaddingIntString(sub_order_index,4," ",true)+"] = "+ exeName );
                        sub_order_index++;
                    }
                    order_index++;
                }
            }
           // ArrayPrint(pathInfoList, "PATH环境变量信息");
        }


    }

    static  ArrayList<File> getPathExeFileName(File listFile){
        ArrayList<File> exeList = new   ArrayList<File> ();
        if(CUR_OS_TYPE == OS_TYPE.Windows){
            ...
            for (int i = 0; i < subFile.length; i++) {
                ...
                if(subFileItem.isFile()){
                    String fileNmae = subFileItem.getName();
                    if(fileNmae.endsWith(".exe") || fileNmae.endsWith(".bat") || fileNmae.endsWith(".cmd") ){
                        exeList.add(subFileItem);
                    }
                }
            }
        }
    ...
    void InitRule(){
        ...
        //   加入类型一一对应的 那些 规则
        CUR_RULE_LIST.add( new Json_Server_Rule_1());
...
//        CUR_RULE_LIST.add( new ClearChineseType_8());
    ...
    // J3_JsonServer_NPM
// 1. 检测当前是否安装了 npm 环境  如果没有 提示 安装 npm.msi文件  可通过 ZSoftWare
// 2. 如果当前 npm 安装成功了  那么 检测 当前是有有安装 json-server ，  如果没有安装 提示安装
// 3. 对输入的json文件进行集成, 以 名字为key, 文件内容为value放入到 J3_json.json 文件中
// 4. 启动 json-server  并 打开当前的 json-server 文件

    class Json_Server_Rule_1 extends  Basic_Rule{
        ArrayList<File> curInputFileList ;

        Json_Server_Rule_1(boolean mIsInputDirAsSearchPoint){
            super(1);
        ...
        boolean checkParamsOK(File shellDir, String type2Param, ArrayList<String> otherParams) {
            ...
            for (int i = 0; i <otherParams.size() ; i++) {
                ...
                if(curFIle.exists() || curFIle.getName().endsWith(".json")){
        ...
        ArrayList<File> applyOperationRule(ArrayList<File> curFileList, HashMap<String, ArrayList<File>> subFileTypeMap, ArrayList<File> curDirList, ArrayList<File> curRealFileList) {
            ...
            ArrayList<String> newJsonList = new  ArrayList<String>();
            newJsonList.add("{");
            for (int i = 0; i < curInputFileList.size(); i++) {
                File fileItem = curInputFileList.get(i);
                System.out.println("index = "+i+"   fileItem = "+ fileItem.getName());
                String fileName_noPoint = getFileNameNoPoint(fileItem.getName());
                String fileContent = ReadFileContent(fileItem);
                String newJsonItem  = "\""+fileName_noPoint+"\""+":"+fileContent;



                if(isjson(fileContent) || isjsonArr(fileContent)){
                    System.out.println("当前输入的JSON文件 "+  fileItem.getName()  +" 添加到 J3_Template.json 模板 文件  OK!  ");
                    newJsonList.add(newJsonItem);
                    newJsonList.add(",");

                }else{
                    System.out.println("当前输入的JSON文件 "+  fileItem.getName()  +" 不是 标准的 json文件!  不会添加到 J3_Template.json 模板 文件 Failed！");
                }

            }
            String lastStr = newJsonList.get(newJsonList.size()-1);
            if(",".equals(lastStr)){
                newJsonList.remove(newJsonList.size()-1);
            }
            newJsonList.add("}");

            writeContentToFile(J3_JSON_File,newJsonList);

            String command = json_server_path + "  --watch " + J3_JSON_File.getAbsolutePath();
            System.out.println("Command:  " + command );
            String openChrome = "chrome     http://localhost:3000/";
        ...
        String ruleTip(String type,int index , String batName,OS_TYPE curType){
            ...
            if(curType == OS_TYPE.Windows){
                itemDesc = batName.trim()+  "<json1.json>  <json2.json>   <json3.json> "+"  描述:"+simpleDesc();
            }else{
        ...
        @Override
        String simpleDesc() {
            return "把指定的 json 文件挂载到 json-server 使得 能在浏览器中访问得到 json格式的数据  http://localhost:3000/  查看当前的json数据";
        }
    }

    static boolean isjsonArr(String string){
        try {
    ...
    static boolean isjson(String string){
        try {
...
/*
            String itemDesc = "";
           if(CUR_OS_TYPE == OS_TYPE.Windows){
                itemDesc = "zrule_apply_J3.bat  "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }else{
               itemDesc = "zrule_apply_J3 "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }
           */
...
//        System.out.println("══════════════End ExE ");
    ...
    static  String json_server_path = null;
    static  String npm_path = null;
    ...
    public static void main(String[] args) {
        ...
        if(!checkNpm()){
            System.out.println("当前 PATH 环境中 不存在 npm 环境 请去 下载安装并添加到环境变量 对于 npm ");
            System.out.println("npm 下载地址:  "+ " https://nodejs.org/en/download/ ");
            System.out.println("静默安装命令示例: "+"start /wait /min node-v14.15.0-x64.msi  /S /Q /D=D:\\Command_InstallSoft\\node-v14.15.0-x64");
            return;
        }else{
            npm_path  = shellName_File_Map.get("npm.cmd").getAbsolutePath();
            System.out.println("当前存在 NPM 环境 : "+ npm_path);
        }

        if(!checkJsonServer()){
            System.out.println("当前 PATH 环境中 存在 npm 环境  但 不存在 json-server 插件 plugin 请执行下载 json-server  操作 ");
            System.out.println("json-server 下载命令:  "+ "   npm install -g json-server  &&  npm init  --yes  &&  npm install -g json-server ");
            return;
        }else{

            json_server_path = shellName_File_Map.get("json-server.cmd").getAbsolutePath();
            System.out.println("当前存在 json-server  环境 : "+ json_server_path);
        }



        J3_JsonServer_NPM mJ3_Object = new J3_JsonServer_NPM();
        mJ3_Object.InitRule();
