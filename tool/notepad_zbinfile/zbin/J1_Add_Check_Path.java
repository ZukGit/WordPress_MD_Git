...
//        System.out.println("══════════════End ExE ");
    ...
    static  void Create_AddPath_Operation(){
        ...
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < install_software_path_list.size(); i++) {
            String absItem = install_software_path_list.get(i);
        ...
        for (int i = 0; i < System_File.size(); i++) {
            ...
            for (int j = 0; j < fileArr.length; j++) {
                ...
                if(targetFile.isDirectory()){
                    ...
                    boolean isSearchDir = isSearch_CustomeDefine(targetFileName);
                    if(isSearchDir){
...
//            System.out.println("PATH缺失_搜索匹配["+paddingIndex+"] = "+loss_match_key.get(i) +"   路径:"+ absFile.getAbsolutePath());
   ...
   static boolean isSearch_CustomeDefine(String dirName){
        boolean falg = true;
       ...
       for (int i = 0; i < systemDirName.size(); i++) {
           ...
           if(curDirName_low.equals(systemNameItem) || curDirName_low.startsWith(systemNameItem)){
               return false;
           }
       }
...
//                System.out.println(pathFile.getAbsolutePath() +"   不存在?  pathItem = "+ pathItem);
        ...
        for (int i = 0; i < OK_PATH_File_LIST.size(); i++) {
            File pathFile = OK_PATH_File_LIST.get(i);
