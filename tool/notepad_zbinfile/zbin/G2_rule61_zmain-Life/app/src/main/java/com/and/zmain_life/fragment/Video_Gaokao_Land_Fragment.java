...
public class Video_Gaokao_Land_Fragment extends BaseFragment {
...
//        videoView = new FullScreenVideoView(getActivity());
        ...
        //监听播放或暂停事件
        RxBus.getDefault().toObservable(Pause_MP4_Gaokao_Land_VideoEvent.class)
                .subscribe((Action1<Pause_MP4_Gaokao_Land_VideoEvent>) event -> {
                    if (event.isPlayOrPause()) {
                        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.init BXXXX   ");
                        playCurVideo(DataHolder.Video_Gaokao_Land_initPos);
                        videoView.start();
...
//                        mediaControl.hide();
                        ...
                        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.init C   ");
                        videoView.pause();
                        if(mediaControl != null){
                            mediaControl.hide();
                        }
    ...
    public void onResume() {
        super.onResume();
        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume  A   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);

        //返回时，推荐页面可见，则继续播放视频
        if (MainActivity.curMainPage == 0 && MainFragment.curPage == curPageIndex) {
            Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume  B   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);
            Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume  CA ");
            new CountDownTimer(200, 200) {
                ...
                public void onFinish() {
                    ...
                    videoView.seekTo(0);
                    android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume FF_1  onFinish          videoView.isPlaying()="+videoView.isPlaying());
                    mediaControl.show();

                    videoView.start();
                    android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume FF_2  onFinish          videoView.isPlaying()="+videoView.isPlaying());
                }
            }.start();
        }
        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume  CB ");
        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onResume  D ");
    ...
    public void onPause() {
        super.onPause();
        Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Gaokao_Fragment.onPause  ");
        videoView.pause();
        ...
        viewPagerLayoutManager.setOnViewPagerListener(new OnViewPagerListener() {
            ...
            public void onInitComplete() {
                ...
                if (MainActivity.curMainPage == 0 && MainFragment.curPage == curPageIndex) {
                    ...
                    playCurVideo(DataHolder.Video_Gaokao_Land_initPos);    //  切到 video_raw 时候 就执行 resume 了 然后 执行到这里
...
//                    ivCurCover.setVisibility(View.VISIBLE);
            ...
            public void onPageSelected(int position, boolean isBottom) {
                ...
                if(mediaControl != null)
                    mediaControl.setVisibility(View.INVISIBLE);
                Log.i("zukgit", "X1 Video_Gaokao_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_initPos  =   " + DataHolder.Video_Gaokao_Land_initPos + " isBottom = " + isBottom);
                boolean up = curPlayPos > position;

                if (up) {
//                    int step = curPlayPos - position;
                    DataHolder.Video_Gaokao_Land_initPos = DataHolder.Video_Gaokao_Land_initPos - 1;
...
//                    int step = position - curPlayPos;
                ...
                if (DataHolder.Video_Gaokao_Land_initPos < 0 || position == 0) {
                    DataHolder.Video_Gaokao_Land_initPos = 0;
                }
                curPlayPos = DataHolder.Video_Gaokao_Land_initPos;
                Log.i("zukgit", "X2  Video_Gaokao_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_initPos  =   " + DataHolder.Video_Gaokao_Land_initPos + " user_backup_num = " + user_backup_num);
    ...
    private void playCurVideo(int position) {
        Log.i("zukgit", "Video_Gaokao A playCurVideo curPlayPos = " + curPlayPos + " position  =" + position);
...
/*        if (position == curPlayPos) {
        ...
        Log.i("zukgit", "Video_Gaokao B playCurVideo  curPlayPos = " + curPlayPos + " position  =" + position + "  viewPagerLayoutManager.getChildCount() = " + viewPagerLayoutManager.getChildCount());
...
//        }
        ...
        Log.i("zukgit", "Video_Gaokao D playCurVideo  curPlayPos = " + curPlayPos + " position  =" + position + " DataCreate.datas.size = " + DataCreate.datas.size());
...
//        ImageView ivCover = rootView.findViewById(R.id.iv_cover);
        ...
        Log.i("zukgit", "Video_Gaokao E playCurVideo  curPlayPos = " + curPlayPos + " position  =" + position);
        curPlayPos = position;

        //切换播放器位置
        dettachParentView(rootView);
        Log.i("zukgit", "Video_Gaokao F playCurVideo  curPlayPos = " + curPlayPos + " position  =" + position);
...
//        autoPlayVideo(curPlayPos,ivCover);
        ...
        Log.i("zukgit", "Video_Gaokao G playCurVideo  curPlayPos = " + curPlayPos + " position  =" + position);
...
//        String fileName = getTimeStamp() + "_" + DataHolder.video_Gaokao_Land_file_list[DataHolder.Video_Gaokao_Land_random_indexlist.get(position)].getName();
//        File jiemiFile = new File(video_file_list[0].getParentFile().getAbsolutePath()+File.separator+fileName);
//        File jiemiFile = EncryUtil.createDecryByteData(DataHolder.video_Gaokao_file_list[DataHolder.Video_Gaokao_random_indexlist.get(position)]);

        File jiemiFile = DataHolder.Video_Gaokao_Land_file_list[DataHolder.Video_Gaokao_Land_random_indexlist.get(position%DataHolder.Video_Gaokao_Land_count)];
            ...
            public boolean onLongClick(View view) {
                if(mediaControl != null){
                    ...
                    if(!isVideoLoop){  // 立即进入下一个页面  并取消当前循环
                        DataHolder.Video_Gaokao_Land_initPos++;
                        curPlayPos = DataHolder.Video_Gaokao_Land_initPos;
//                android.util.Log.i("zukgit","B curPlayPos = "+ curPlayPos);
                        Log.i("zukgit", "X2 往下轮询  Video_Gaokao_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Gaokao_initPos  =   " + DataHolder.Video_Gaokao_Land_initPos);

                        videoView.stopPlayback();
//                autoPlayVideo(curPlayPos, ivCover);
                        recyclerView.scrollToPosition(DataHolder.Video_Gaokao_Land_initPos);
        ...
        videoView.setOnTouchListener( new View.OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                Log.i("zukgit", "Gaokao_Video_Fragment setOnTouchListener  motionEvent ="+ motionEvent.getAction() + "_Pretime  ="+ Action_UP_Pretime);
                switch (motionEvent.getAction()){
                    case MotionEvent.ACTION_DOWN:  // 0
                        ...
                        if(distance < time_space){
                            Log.i("zukgit", "Gaokao_Video_Fragment setOnTouchListener  motionEvent ="+ motionEvent.getAction() + "  Pretime"+ Action_UP_Pretime +  "   distance = "+ distance);

                            Action_UP_Pretime = System.currentTimeMillis();
                            if(mediaControl !=null ){
                                if(mediaControl.getVisibility() != View.VISIBLE){
                                    mediaControl.setVisibility(View.VISIBLE);
                                    Log.i("zukgit", "Gaokao_Video_Fragment setOnTouchListener  motionEvent ="+ motionEvent.getAction() + "mediaControl.setVisibility(View.VISIBLE)  Pretime"+ Action_UP_Pretime +  "   distance = "+ distance);
        ...
        videoView.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(MediaPlayer mp) {
                ...
                if(isVideoLoop) {   // 单曲循环
                    ...
                    Log.i("zukgit", "X1 单曲循环  Video_Gaokao_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Gaokao_initPos  =   " + DataHolder.Video_Gaokao_Land_initPos);
...
//                        return true;
    ...
    void showDeleteFileDialog(File mFile , int fileIndex ){
        ...
        alterDiaglog.setTitle("删除文件:"+getPageTip()+"["+((fileIndex)%DataHolder.Video_Gaokao_Land_count)+"]"+"["+DataHolder.Video_Gaokao_Land_count+"]"+mFile.getAbsolutePath());//文字
//        alterDiaglog.setMessage("请确认删除以下文件:\n"+mFile.getAbsolutePath());//提示消息
        ...
        //  ______________  Video_Gaokao_Land   SingleChoiceItems Dialog Begin ______________
        ArrayList<String> singleListAr = new ArrayList<String>();

        for (int i = 0; i < DataHolder.Video_Gaokao_Land_KeyNameList.size() ; i++) {
            String ssKey = DataHolder.Video_Gaokao_Land_KeyNameList.get(i);
            ArrayList<File> matchFileArr =    DataHolder.Video_Gaokao_Land_Category_FileArr_Map.get(ssKey);
            if(matchFileArr != null && matchFileArr.size() > 0){
                print("TopFragment  Tokkey["+i+"]["+DataHolder.Video_Gaokao_Land_KeyNameList.size()+"] = "+ ssKey+" ArrSize="+matchFileArr.size());
...
//                singleListAr.add(realName+"_"+matchFileArr.size());
                ...
                print("TopFragment  SSkey["+i+"]["+DataHolder.Video_Gaokao_Land_KeyNameList.size()+"] = "+ ssKey+" ArrSize=0");
            }
...
//                print("SSFragment SSkey num1Str="+num1Str+"    num2Str="+num2Str);
        ...
        for (int i = 0; i < singleListAr.size(); i++) {
            ...
            if(categoryKey.startsWith(DataHolder.Video_Gaokao_Land_MapKey)){
                matchedCategory = i ;
            }
        ...
        alterDiaglog.setSingleChoiceItems(items,matchedCategory, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                DataHolder.Video_Gaokao_Land_MapKey = categoryName+"";  //all_7
                String keyName = DataHolder.Video_Gaokao_Land_KeyName_KeyNameSize_Map.get(categoryName);  //all
                ArrayList<File> matchFileArr =    DataHolder.Video_Gaokao_Land_Category_FileArr_Map.get(keyName);
                if(matchFileArr == null){
                    ...
                    curDialog.dismiss();
                    DataHolder.showStringMap(DataHolder.Video_Gaokao_Land_KeyName_KeyNameSize_Map);
                    return;

                }
                Toast.makeText(getContext(),"ZTopCategoryName【"+categoryName+"】 keyName["+keyName+"] Size["+matchFileArr.size()+"]",Toast.LENGTH_SHORT).show();

                DataHolder.refresh_Video_Gaokao_Land_List(matchFileArr);
...
//  ______________  Video_Gaokao_Land   SingleChoiceItems Dialog End ______________
        //中立的选择
        alterDiaglog.setNeutralButton("删除", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(fileIndex == 0){
                    ...
                    DataHolder.Video_Gaokao_Land_file_list[DataHolder.Video_Gaokao_Land_random_indexlist.get((fileIndex)%DataHolder.Video_Gaokao_Land_count)] =      DataHolder.Video_Gaokao_Land_file_list[DataHolder.Video_Gaokao_Land_random_indexlist.get((fileIndex+1)%DataHolder.Video_Gaokao_Land_count)];


                }   else    if(fileIndex > 0 && fileIndex < DataHolder.Video_Gaokao_Land_file_list.length){
