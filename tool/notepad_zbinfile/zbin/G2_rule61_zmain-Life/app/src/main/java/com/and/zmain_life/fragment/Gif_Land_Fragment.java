...
public class Gif_Land_Fragment extends BaseFragment {
...
//        recyclerView.setAdapter(adapter);
        ...
        mGifNewAdapter = new GifNewAdapter(getActivity(), DataHolder.Top_Land_Gif_random_indexlist,DataHolder.Top_Land_Gif_file_list);
...
// s少时诵诗书所
        ...
        //监听播放或暂停事件
        RxBus.getDefault().toObservable(Pause_Gif_Land_Event.class)
                .subscribe((Action1<Pause_Gif_Land_Event>) event -> {
                    if (event.isPlayOrPause()) {
                        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Pause_TopLand_GifEvent.init B   ");
                        pause_gif(false);
                        gotoNextPage_loop(false);


                    } else {
                        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Pause_TopLand_GifEvent.init C   ");
                        pause_gif(true);
                    }
                });
        pause_gif(true);
    ...
    public void onResume() {
        super.onResume();
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Top_Land_GifFragment.onResume  A   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);
...
/*        //返回时，推荐页面可见，则继续播放视频
        ...
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Top_Land_GifFragment.onResume  CB ");
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Top_Land_GifFragment.onResume  D ");
    ...
    public void onPause() {
        super.onPause();
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Top_Land_RecommendFragment.onPause  ");
        pause_gif(true);
//        videoView.pause();
    }
    ...
    private void setViewPagerLayoutManager() {
        ...
        recyclerView.scrollToPosition(DataHolder.Top_Land_Gif_initPos);
        viewPagerLayoutManager.setOnViewPagerListener(new OnViewPagerListener() {
            ...
            public void onInitComplete() {
                ...
                playCurGif(DataHolder.Top_Land_Gif_initPos);
            }
            ...
            public void onPageSelected(int position, boolean isBottom) {
                ...
                android.util.Log.i("zukgit", "X1 GifFragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Top_Land_Gif_initPos  =   " + DataHolder.Top_Land_Gif_initPos + " isBottom = " + isBottom);
                boolean up = curPlayPos > position;

                if (up) {
//                    int step = curPlayPos - position;
                    DataHolder.Top_Land_Gif_initPos = DataHolder.Top_Land_Gif_initPos - 1;
...
//                    int step = position - curPlayPos;
                ...
                if (DataHolder.Top_Land_Gif_initPos < 0 || position == 0) {
                    DataHolder.Top_Land_Gif_initPos = 0;
                }
                DataHolder.Top_Land_Gif_initPos =  position;

                curPlayPos = DataHolder.Top_Land_Gif_initPos;
                android.util.Log.i("zukgit", "X2  Top_Land_GifFragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Top_Land_Gif_initPos  =   " + DataHolder.Top_Land_Gif_initPos + " user_backup_num = " + user_backup_num);
...
//        String fileName = getTimeStamp() + "_" + DataHolder.Top_Land_gif_file_list[DataHolder.Top_Land_Gif_random_indexlist.get(position)].getName();
//        File jiemiFile = new File(video_file_list[0].getParentFile().getAbsolutePath()+File.separator+fileName);
//        File jiemiFile = EncryUtil.createDecryByteData(DataHolder.Top_Land_gif_file_list[DataHolder.Top_Land_Gif_random_indexlist.get(position)]);

        File jiemiFile = DataHolder.Top_Land_Gif_file_list[DataHolder.Top_Land_Gif_random_indexlist.get(position%DataHolder.Top_Land_Gif_count)];
...
//                android.util.Log.i("zukgit","B curPlayPos = "+ curPlayPos);
                android.util.Log.i("zukgit", "X2  RecommendFragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Top_Land_Gif_initPos  =   " + DataHolder.Top_Land_Gif_initPos);

                videoView.stopPlayback();
//                autoShowGif(curPlayPos, ivCover);
                autoShowGif(DataHolder.Top_Land_Gif_initPos);
            }
        });*/
...
/*    void wait_1second_operation(){
    ...
    new CountDownTimer(1000, 500) {
        ...
        @Override
        public void onFinish() {
            if(curPlayPos == DataHolder.Top_Land_Gif_initPos){
                pause_gif(false);
            }
        }
    }.start();
        ...
        if(!isGifPause){
            ...
            loopPlay_Timer =    new CountDownTimer(6000, 500) {
                ...
                public void onFinish() {
                    ...
                    if(isGifPause || curPlayPos == DataHolder.Top_Land_Gif_initPos) {
                        DataHolder.Top_Land_Gif_initPos++;
//                    playCurGif(DataHolder.Top_Land_Gif_initPos);
//                        recyclerView.scrollToPosition(DataHolder.Top_Land_Gif_initPos);
...
//                            Toast.makeText(getContext(),"ImageView_MotionEvent.ACTION_POINTER_DOWN "+event.getPointerCount()+"\n"+"FileName:"+rawFile.getAbsolutePath(),Toast.LENGTH_SHORT).show();
    ...
    void showDeleteFileDialog(File mFile , int fileIndex ){
        ...
        alterDiaglog.setTitle("删除文件:"+getPageTip()+"["+((fileIndex)%DataHolder.Top_Land_Gif_count)+"]"+"["+DataHolder.Top_Land_Gif_count+"]");//文字
        ...
        alterDiaglog.setNeutralButton("删除", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(fileIndex == 0){
                    ...
                    DataHolder.Top_Land_Gif_file_list[DataHolder.Top_Land_Gif_random_indexlist.get((fileIndex)%DataHolder.Top_Land_Gif_count)] =      DataHolder.Top_Land_Gif_file_list[DataHolder.Top_Land_Gif_random_indexlist.get((fileIndex+1)%DataHolder.Top_Land_Gif_count)];


                } else if(fileIndex > 0 && fileIndex < DataHolder.Top_Land_Gif_file_list.length){
