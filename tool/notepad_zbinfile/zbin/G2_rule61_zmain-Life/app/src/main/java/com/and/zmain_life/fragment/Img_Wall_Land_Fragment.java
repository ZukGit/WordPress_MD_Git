...
public class Img_Wall_Land_Fragment extends BaseFragment {
...
//        adapter = new ImgAdapter(getActivity(), DataCreate.datas);
        ...
        JpgAdapter = new JPGAdapter(getActivity(), DataHolder.Wall_Land_Img_random_indexlist,DataHolder.Wall_Land_Img_file_list);
...
// s少时诵诗书所
        ...
        //监听播放或暂停事件
        RxBus.getDefault().toObservable(Pause_Wall_Land_ImgEvent.class)
                .subscribe((Action1<Pause_Wall_Land_ImgEvent>) event -> {
                    if (event.isPlayOrPause()) {
                        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " ImgWallLandFragment.init B   ");
                        mDoubleTap_ScaleNum_Index = 0;
                        android.util.Log.i("zukgit", "AA toObservable  DataHolder.Top_Land_Img_initPos = " + DataHolder.Wall_Land_Img_initPos + " pause_Img(true)   ");
                        if(recyclerView != null){
                            recyclerView.scrollToPosition(DataHolder.Wall_Land_Img_initPos);
                            android.util.Log.i("zukgit", "AA toObservable  DataHolder.Top_Land_Img_initPos = " + DataHolder.Wall_Land_Img_initPos + " pause_Img(true)   ");
                        }
                        pause_Img(false);
                        ...
                        android.util.Log.i("zukgit", "BB toObservable  DataHolder.Top_Land_Img_initPos = " + DataHolder.Wall_Land_Img_initPos + " pause_Img(true)   ");
                        pause_Img(true);
                    }
                });
        pause_Img(true);
    ...
    public void onResume() {
        super.onResume();
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " ImgWallLandFragment.onResume  A   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);
...
/*        //返回时，推荐页面可见，则继续播放视频
        ...
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " ImgWallLandFragment.onResume  CB ");
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " ImgWallLandFragment.onResume  D ");
...
if(DataHolder.Wall_Land_Img_random_indexlist == null){
    return;
}
                File jiemiFile = EncryUtil.createDecryByteData(DataHolder.Wall_Land_Img_file_list[DataHolder.Wall_Land_Img_random_indexlist.get(DataHolder.Wall_Land_Img_initPos)]);
                Uri uri_item = Uri.fromFile(jiemiFile);
                android.util.Log.i("zukgit", "onInitComplete ivCover.setImageURI    curPlayPos =" + curPlayPos + "DataHolder.Top_Land_Img_initPos ="+ DataHolder.Wall_Land_Img_initPos + "uri_item = "+ uri_item.toString());

                playCurImg(DataHolder.Wall_Land_Img_initPos);
            }
            ...
            public void onPageSelected(int position, boolean isBottom) {
                ...
                android.util.Log.i("zukgit", "X1 ImgFragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Top_Land_Img_initPos  =   " + DataHolder.Wall_Land_Img_initPos + " isBottom = " + isBottom);
...
//                    int step = curPlayPos - position;
                    DataHolder.Wall_Land_Img_initPos = DataHolder.Wall_Land_Img_initPos - 1;
...
//                    int step = position - curPlayPos;
                ...
                if (DataHolder.Wall_Land_Img_initPos < 0 || position == 0) {
                    DataHolder.Wall_Land_Img_initPos = 0;
                }
                DataHolder.Wall_Land_Img_initPos =    position ;
                curPlayPos = DataHolder.Wall_Land_Img_initPos;
                android.util.Log.i("zukgit", "X2  ImgFragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Top_Land_Img_initPos  =   " + DataHolder.Wall_Land_Img_initPos + " user_backup_num = " + user_backup_num);
...
//        File jiemiFile = EncryUtil.createDecryByteData(DataHolder.Wall_Land_Img_file_list[DataHolder.Wall_Land_Img_random_indexlist.get(position)]);

        File jiemiFile = DataHolder.Wall_Land_Img_file_list[DataHolder.Wall_Land_Img_random_indexlist.get(position%DataHolder.Wall_Land_Img_count)];

        Uri uri_item = Uri.fromFile(jiemiFile);
        android.util.Log.i("zukgit", "ivCover.setImageURI  position ="+ position + "   DataHodler.ImgiIndex="+DataHolder.Wall_Land_Img_initPos + "   Img Pre  =" + uri_item.toString());
...
/*    void wait_1second_operation(){
    ...
    void gotoNextPage_loop(boolean imediate){
        ...
        if(!isImgPause){
            ...
            if(imediate){
                DataHolder.Wall_Land_Img_initPos++;
                android.util.Log.i("zukgit", "AA DataHolder.Top_Land_Img_initPos   DataHolder.Top_Land_Img_initPos = " + DataHolder.Wall_Land_Img_initPos);
                recyclerView.scrollToPosition(DataHolder.Wall_Land_Img_initPos);
            }
            if(loopPlay_Timer != null){
                loopPlay_Timer.cancel();
            ...
            loopPlay_Timer =    new CountDownTimer(3500, 500) {
                ...
                public void onFinish() {
                    ...
                    if(!isImgPause) {
                        DataHolder.Wall_Land_Img_initPos++;
                        onFinish_num++;
                        android.util.Log.i("zukgit", "【"+onFinish_num+ "】 = onFinish_num   DataHolder.Top_Land_Img_initPos   DataHolder.Top_Land_Img_initPos = " + DataHolder.Wall_Land_Img_initPos);
...
//                            Toast.makeText(getContext(),"ImageView_MotionEvent.ACTION_POINTER_DOWN "+event.getPointerCount()+"\n"+"FileName:"+rawFile.getAbsolutePath(),Toast.LENGTH_SHORT).show();
    ...
    void showDeleteFileDialog(File mFile , int fileIndex ){
        ...
        alterDiaglog.setTitle("删除文件:"+getPageTip()+"["+((fileIndex)%DataHolder.Wall_Land_Img_count)+"]"+"["+DataHolder.Wall_Land_Img_count+"]");//文字
        ...
        alterDiaglog.setNeutralButton("删除", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(fileIndex == 0){
                    ...
                    DataHolder.Wall_Land_Img_file_list[DataHolder.Wall_Land_Img_random_indexlist.get((fileIndex)%DataHolder.Wall_Land_Img_count)] =      DataHolder.Wall_Land_Img_file_list[DataHolder.Wall_Land_Img_random_indexlist.get((fileIndex+1)%DataHolder.Wall_Land_Img_count)];


                } else        if(fileIndex > 0 && fileIndex < DataHolder.Wall_Land_Img_file_list.length){
