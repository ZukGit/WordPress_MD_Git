...
public class Stock_MultiLevelPickerWindow<T extends Stock_Node> extends PopupWindow {
    ...
    public Stock_MultiLevelItemAdapter<T> mAdapter1, mAdapter2, mAdapter3;

    /**
     * 储存已选择的数据
     */
    public long[] storage = new long[]{-1L/* 全部 */, -1L, -1L};
    ...
    public void click_level_1(Stock_NodeImpl rootImpl) {

        if(mAdapter1 != null){
            rootImpl.setSelectedChild(Stock_MultiLevelItemAdapter.last_click_level1_ID);

        }


        if(rv2 != null && mAdapter2 != null){

            rv2.requestLayout();
            rv2.scrollToPosition(0);
            Log.i("zukigt","A click_level_1  mAdapter2 = "+ mAdapter2  +"  rv2="+rv2);
        }

        long[] storage_new_id = new long[]{Stock_MultiLevelItemAdapter.last_click_level1_ID /* 全部 */, Stock_MultiLevelItemAdapter.last_click_level2_ID, -1L};

        storage =  storage_new_id ;
    ...
    public Stock_MultiLevelPickerWindow(Context context , int colorId) {
        mBGColorId = colorId;
        init(context);
    }
    ...
    private void init(final Context context) {
        ...
        mRootView = LayoutInflater.from(context).inflate(
                R.layout.stock_window_picker, null);
        mcontext = context;
        buildView(mRootView);
...
public void refreshLevel3Adapter(Stock_Node stockImpl){
...
/*        rv1.setBackgroundColor(mBGColorId);
        ...
        mAdapter1 = new Stock_MultiLevelItemAdapter<>(null, 0,mcontext,this,1);
        ...
        mAdapter2 = new Stock_MultiLevelItemAdapter<>(null, 1,mcontext,this,2);
        rv2.setAdapter(mAdapter2);
        ...
        mAdapter3 = new  Stock_MultiLevelItemAdapter<>(null, 2,mcontext,this,3);
...
//        re.fillInStackTrace();
//        android.util.Log.v("zukgit_stock_dismiss", "RuntimeException", re);
...
//            this.setHeight(ScreenUtil.getScreenHeight(context));//屏幕的高
            ...
            if (mListener != null)
                ...
                this.dismiss();   // 不 dismiss


        }
    }

    public void show(View view,boolean isShowWindows_When_Dismiss) {
        if (!this.isShowing()) {
//            PopupWindowCompat.showAsDropDown(this, view, 0, 0, Gravity.END);
