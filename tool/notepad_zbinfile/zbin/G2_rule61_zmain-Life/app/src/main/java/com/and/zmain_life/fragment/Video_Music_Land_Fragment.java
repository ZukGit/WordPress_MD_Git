...
public class Video_Music_Land_Fragment extends BaseFragment {
    String TAG = "Video_Music_Land_Fragment";
...
//        videoView = new FullScreenVideoView(getActivity());
        ...
        //监听播放或暂停事件
        RxBus.getDefault().toObservable(Pause_MP4_Music_Land_VideoEvent.class)
                .subscribe((Action1<Pause_MP4_Music_Land_VideoEvent>) event -> {
                    if (event.isPlayOrPause()) {
                        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.init B   ");
                        playCurVideo(DataHolder.Video_Music_Land_initPos);
                        videoView.start();
...
//                        mediaControl.hide();
                        ...
                        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.init C   ");
                        videoView.pause();
                        if(mediaControl != null){
                            mediaControl.hide();
                        }
    ...
    public void onResume() {
        super.onResume();
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume  A   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);

        //返回时，推荐页面可见，则继续播放视频
        if (MainActivity.curMainPage == 0 && MainFragment.curPage == curPageIndex) {
            android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume  B   MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage);
            android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume  CA ");
            new CountDownTimer(200, 200) {
                ...
                public void onFinish() {
                    ...
                    videoView.seekTo(0);
                    android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume FF_1  onFinish          videoView.isPlaying()="+videoView.isPlaying());
                    mediaControl.show();

                    videoView.start();
                    android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume FF_2  onFinish          videoView.isPlaying()="+videoView.isPlaying());
                }
            }.start();
        }else {  // 当前 执行了 Resume 但还不是主界面

            android.util.Log.i("zukgit0509_1", " Video_Music_Land_Fragment.onResume  非可见页面Resume    MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage +"   curPageIndex="+curPageIndex);

            if(videoView != null ){
                boolean isPalying =  videoView.isPlaying();
                android.util.Log.i("zukgit0509_1", " Video_Music_Land_Fragment.onResume  非可见页面Resume   isPalying="+isPalying+"  MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage +"   curPageIndex="+curPageIndex);

                if(isPalying){
                    videoView.pause();
                }
            } else{
                android.util.Log.i("zukgit0509_1", " Video_Music_Land_Fragment.onResume  非可见页面Resume videoView为空    MainActivity.curMainPage =" + MainActivity.curMainPage + "  MainFragment.curPage =" + MainFragment.curPage +"   curPageIndex="+curPageIndex);

            }

        }
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume  CB ");
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onResume  D ");
    ...
    public void onPause() {
        super.onPause();
        android.util.Log.i("zukgit", "curPlayPos = " + curPlayPos + " Video_Music_Land_Fragment.onPause  ");
        videoView.pause();
    ...
    private void setViewPagerLayoutManager() {
        ...
        viewPagerLayoutManager.setOnViewPagerListener(new OnViewPagerListener() {
            ...
            public void onInitComplete() {
                android.util.Log.i("zukgit", "Pause_Music_Video_Music_Land_initPos VideoEvent onInitComplete = " + curPlayPos + " position  = 1 A ");
//                playCurVideo(DataHolder.Video_Music_Land_initPos);
                if (MainActivity.curMainPage == 0 && MainFragment.curPage == curPageIndex) {

                    recyclerView.scrollToPosition(DataHolder.Video_Music_Land_initPos);
                    playCurVideo(DataHolder.Video_Music_Land_initPos);
                }
...
//                    ivCurCover.setVisibility(View.VISIBLE);
            ...
            public void onPageSelected(int position, boolean isBottom) {
                ...
                android.util.Log.i("zukgit", "X1 Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos + " isBottom = " + isBottom);
                boolean up = curPlayPos > position;

                if (up) {
//                    int step = curPlayPos - position;
                    DataHolder.Video_Music_Land_initPos = DataHolder.Video_Music_Land_initPos - 1;
...
//                    int step = position - curPlayPos;
                ...
                if (DataHolder.Video_Music_Land_initPos < 0 || position == 0) {
                    DataHolder.Video_Music_Land_initPos = 0;
                }
                curPlayPos = DataHolder.Video_Music_Land_initPos;
                android.util.Log.i("zukgit", "X2  Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos + " user_backup_num = " + user_backup_num);
...
//        String bgVideoPath = "android.resource://" + getActivity().getPackageName() + "/" + DataCreate.datas.get(position).getVideoRes();
        ...
        //  String fileName = getTimeStamp() + "_" + DataHolder.Video_Music_Land_file_list[DataHolder.Video_Music_Land_random_indexlist.get(position)].getName();
//        File jiemiFile = new File(video_file_list[0].getParentFile().getAbsolutePath()+File.separator+fileName);
//        File jiemiFile = EncryUtil.createDecryByteData(DataHolder.Video_Music_Land_file_list[DataHolder.Video_Music_random_indexlist.get(position)]);

        File    jiemiFile = DataHolder.Video_Music_Land_file_list[DataHolder.Video_Music_Land_random_indexlist.get(position%DataHolder.Video_Music_Land_count)];
            ...
            public boolean onLongClick(View view) {
                if(mediaControl != null){
                    ...
                    if(!isVideoLoop){  // 立即进入下一个页面  并取消当前循环
                        DataHolder.Video_Music_Land_initPos++;
                        curPlayPos = DataHolder.Video_Music_Land_initPos;
//                android.util.Log.i("zukgit","B curPlayPos = "+ curPlayPos);
                        android.util.Log.i("zukgit", "长按 X2 往下轮询  Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);

                        videoView.stopPlayback();
//                autoPlayVideo(curPlayPos, ivCover);
                        recyclerView.scrollToPosition(DataHolder.Video_Music_Land_initPos);
//                        autoPlayVideo(DataHolder.Video_Music_Land_initPos);
                        android.util.Log.i("zukgit", "长按 XX2 往下轮询  Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);

                    }else{
                        android.util.Log.i("zukgit", "长按 X3 单曲循环 Video_Music_Land_Fragment setOnLongClickListener   Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);

                    }

                    android.util.Log.i("zukgit", "长按 X4 setOnLongClickListener   Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);
                    return true;
        ...
        videoView.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(MediaPlayer mp) {
                ...
                if(isVideoLoop){
                    ...
                    android.util.Log.i("zukgit", "X1 单曲循环  Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);

                }else{
                    DataHolder.Video_Music_Land_initPos++;
                    curPlayPos = DataHolder.Video_Music_Land_initPos;
//                android.util.Log.i("zukgit","B curPlayPos = "+ curPlayPos);
                    android.util.Log.i("zukgit", "X2 往下轮询 Video_Music_Land_Fragment.onPageSelected.curPlayPos = " + curPlayPos + " position  =   " + position + " DataHolder.Video_Music_Land_initPos  =   " + DataHolder.Video_Music_Land_initPos);
...
//                        return true;
    ...
    void showDeleteFileDialog(File mFile , int fileIndex ){
        ...
        alterDiaglog.setTitle("删除文件:"+getPageTip()+"["+((fileIndex)%DataHolder.Video_Music_Land_count)+"]"+"["+DataHolder.Video_Music_Land_count+"]"+mFile.getAbsolutePath());//文字
...
//  ______________  Video_Music_Land   SingleChoiceItems Dialog Begin ______________
        ArrayList<String> singleListAr = new ArrayList<String>();

        for (int i = 0; i < DataHolder.Video_Music_Land_KeyNameList.size() ; i++) {
            String ssKey = DataHolder.Video_Music_Land_KeyNameList.get(i);
            ArrayList<File> matchFileArr =    DataHolder.Video_Music_Land_Category_FileArr_Map.get(ssKey);
            if(matchFileArr != null && matchFileArr.size() > 0){
                print("Video_Music_Land_Fragment  Tokkey["+i+"]["+DataHolder.Video_Music_Land_KeyNameList.size()+"] = "+ ssKey+" ArrSize="+matchFileArr.size());
...
//                singleListAr.add(realName+"_"+matchFileArr.size());
                ...
                print("Video_Music_Land_Fragment  SSkey["+i+"]["+DataHolder.Video_Music_Land_KeyNameList.size()+"] = "+ ssKey+" ArrSize=0");
            }
...
//                print("SSFragment SSkey num1Str="+num1Str+"    num2Str="+num2Str);
        ...
        for (int i = 0; i < singleListAr.size(); i++) {
            ...
            if(categoryKey.startsWith(DataHolder.Video_Music_Land_MapKey)){
                matchedCategory = i ;
            }
            items[i] = little_index_str+singleListAr.get(i);
            print("Video_Music_Land_Fragment  Topkey items["+i+"] = "+items[i]  );
        }
        ...
        alterDiaglog.setSingleChoiceItems(items,matchedCategory, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                DataHolder.Video_Music_Land_MapKey = categoryName+"";  //all_7
                String keyName = DataHolder.Video_Music_Land_KeyName_KeyNameSize_Map.get(categoryName);  //all
                ArrayList<File> matchFileArr =    DataHolder.Video_Music_Land_Category_FileArr_Map.get(keyName);
                if(matchFileArr == null){
                    ...
                    curDialog.dismiss();
                    DataHolder.showStringMap(DataHolder.Video_Music_Land_KeyName_KeyNameSize_Map);
                    return;

                }
                Toast.makeText(getContext(),"ZTopCategoryName【"+categoryName+"】 keyName["+keyName+"] Size["+matchFileArr.size()+"]",Toast.LENGTH_SHORT).show();

                DataHolder.refresh_Video_Music_Land_List(matchFileArr);
...
//  ______________  Video_Music_Land   SingleChoiceItems Dialog End ______________
        //中立的选择
        alterDiaglog.setNeutralButton("删除", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if(fileIndex == 0){
                    ...
                    DataHolder.Video_Music_Land_file_list[DataHolder.Video_Music_Land_random_indexlist.get((fileIndex)%DataHolder.Video_Music_Land_count)] =      DataHolder.Video_Music_Land_file_list[DataHolder.Video_Music_Land_random_indexlist.get((fileIndex+1)%DataHolder.Video_Music_Land_count)];


                } else      if(fileIndex > 0 && fileIndex < DataHolder.Video_Music_Land_file_list.length){
