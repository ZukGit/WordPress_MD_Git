...
public class NodeImpl implements Node, Serializable {
    ...
    static public final transient NodeImpl EMPTY = new NodeImpl(0,-1, "全部", 0,0,null);


    public NodeImpl(long id, long xparentid , String name, int count,int level , String xmp3Path) {
        this.id = id;
    ...
    public String text() {
        if(level == 3){
            if(DataHolder.isShowLevel3Node_Position){
                String firstChar = DataHolder.getFirstZiMu(name);
                return firstChar+"_"+name+"_"+index4parent;
...
//                return name+"_"+DataHolder.getOriginNode2NameWithName(name);
    ...
    public String json() {
        ...
        if(children == null || children.size() == 0) {
            ...
            sb_json.append("\"children\": "+"[\n");
            for (int i = 0; i < children.size(); i++) {
                NodeImpl childNode = children.get(i);
    ...
    public void addChildren(NodeImpl oneChildren) {
