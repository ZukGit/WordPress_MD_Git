...
//
public class K0_Exe_Install_Tip {


    /*******************修改属性列表 ------Begin *********************/
// 修改0.   全局修改 把 K0 改为当前应用的序号规则序号  当前类名称也需要修改
// 修改1.当前 执行代码的 bat sh 文件名称  最后必须是标识序号
    //修改2. K0_Exe_Install_Tip  改为当前类名称  A1 Z9
    //修改2.1  G2 改为 对应的 标识符
// 修改3.  当前是否有默认的规则   如果有默认的规则那么设置 CUR_TYPE_INDEX为对应index , 没有默认规则那么设置为默认的1
    static String Cur_Bat_Name = "zexe_install_tip_K0";
...
//固定1  zbin 的 字符串绝对路径
    ...
    static File K0_Properties_File = new File(System.getProperties().getProperty("user.home") + File.separator + "Desktop" + File.separator + "zbin" + File.separator + get_Bat_Sh_FlagNumber(Cur_Bat_Name)+".properties");
    static InputStream K0_Properties_InputStream;
    static OutputStream K0_Properties_OutputStream;
    static Properties K0_Properties = new Properties();
...
//  固定属性列表 ------End
    ...
    // ffmpeg  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\K0_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output2.mp4
    // D:\software\ffmpeg\bin
    // D:\software\ffmpeg\bin\ffmpeg.exe  -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\K0_1_MergedRule.txt -c copy C:\Users\zhuzj5\Desktop\output3.mp4
    ...
    static {
        try {
            if (!K0_Properties_File.exists()) {
                K0_Properties_File.createNewFile();
            }
            K0_Properties_InputStream = new BufferedInputStream(new FileInputStream(K0_Properties_File.getAbsolutePath()));
            K0_Properties.load(K0_Properties_InputStream);
            Iterator<String> it = K0_Properties.stringPropertyNames().iterator();
            while (it.hasNext()) {
                String key = it.next();
                // System.out.println("key:" + key + " value: " + K0_Properties.getProperty(key));
                propKey2ValueList.put(key, K0_Properties.getProperty(key));
            }
            K0_Properties_InputStream.close();
    ...
    static void setProperity() {
        try {
            K0_Properties_OutputStream = new BufferedOutputStream(new FileOutputStream(K0_Properties_File.getAbsolutePath()));
            K0_Properties.store(K0_Properties_OutputStream, "");
            K0_Properties_OutputStream.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
/*
            String itemDesc = "";
           if(CUR_OS_TYPE == OS_TYPE.Windows){
                itemDesc = "zrule_apply_K0.bat  "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }else{
               itemDesc = "zrule_apply_K0 "+type+"_"+index + "    [索引 "+count+"]  描述:"+desc;
           }
           */
...
//        System.out.println("══════════════End ExE ");
    ...
    public static ArrayList<File>  calculInputFile(String dirShell , String firstParam , ArrayList<String> otherStrList) {

        ArrayList<File> inputFileList = new   ArrayList<File>();
       File firstFile =  calculInputFile(dirShell,firstParam);
if(firstFile != null  && firstFile.exists() ){
    inputFileList.add(firstFile);

}
        for (int i = 0; i < otherStrList.size() ; i++) {
            String pathItem = otherStrList.get(i);
            File fileItem =  calculInputFile(dirShell,pathItem);
            if(firstFile != null && firstFile.exists() ){
                inputFileList.add(fileItem);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        K0_Exe_Install_Tip mK0_Object = new K0_Exe_Install_Tip();
        mK0_Object.InitRule();



        System.out.println("CUR_Dir_1_PATH = "+ CUR_Dir_1_PATH);
        System.out.println("CUR_TYPE_2_ParamsStr = "+ CUR_TYPE_2_ParamsStr);


        for (int i = 0; i < CUR_INPUT_3_ParamStrList.size(); i++) {
            String inputStr = CUR_INPUT_3_ParamStrList.get(i);
            System.out.println("index["+i+"] = "+ inputStr);
        }
...
//                System.out.println("fileItem.getParent()"+fileItem.getParent() + "  parentPath = "+ parentPath);
