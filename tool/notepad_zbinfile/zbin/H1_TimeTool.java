...
//固定1  zbin 的 字符串绝对路径
    ...
    static ArrayList<String> CUR_INPUT_DeleteParamStrList = new ArrayList<>();
...
//                         System.out.println("file found at path: " + file.toAbsolutePath());
    ...
    static void showNoTypeTip(H1_TimeTool timetool) {
        ...
        System.out.println();
        System.out.println("提示6:  " + Cur_Bat_Name + "  day_XXXXXXXXXX            ## 记录一个日志 该日志的时间为今天   ");
        System.out.println("提示6:  " + Cur_Bat_Name + "  month_YYYYYYYY            ## 记录一个月志 该日志的时间为今天   ");
        System.out.println("提示6:  " + Cur_Bat_Name + "  day[0208]_XXXXXXXXXX            ## 记录一个日志 该日志的时间0208   默认为今年 0208");
        System.out.println("提示6:  " + Cur_Bat_Name + "  month[5]_YYYYYYYY               ## 记录一个月志 该日志的时间为5月  默认今年为5月1日   ");

        System.out.println();
        System.out.println("提示7:  " + Cur_Bat_Name + "  everymonth[1]_AAAAAA             ## 记录一个循环每月日志 日志标识的时间是每个月的 1 号 ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  everymonth[31]_BBBBB              ## 记录一个循环每月日志 日志标识的时间是每个月的 31 号 ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  everyweek[1]_CCCCCC            ## 记录一个循环每周日志  该日志的时间为每周周一   ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  everyweek[7]_DDDDDD            ## 记录一个循环每周日志  该日志的时间为每周周天   ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  everyweek[11-7]_GGGGGG            ## 记录一个指定月份的循环每周日志  该日志的时间为11月的每周周天   ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  everyyear[0207]_EEEEE            ## 记录一个每年的日志 该日志的时间0207    效果同 day[0207]_XXXXX");
...
//clearday0812
        ...
        System.out.println();
        System.out.println("提示7:  " + Cur_Bat_Name + "  clearAll             ##  清除所有 Record 记录");
        System.out.println("提示7:  " + Cur_Bat_Name + "  clearmonth1              ## 清除指定月份 1月 的数据 ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  clearmonth12            ## 清除指定月份 12月 的数据  ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  clearweek12-5           ## 清除指定月份-周数 第12月第5周的数据   ");
        System.out.println("提示7:  " + Cur_Bat_Name + "  clearweek1-1            ## 清除指定月份-周数 第1月第1周的数据    ");
...
//      System.out.println(" Axiniq = "+ xiniq);
    ...
    static boolean isDeleteRecordOperation = false;

    static ArrayList<Record> allRecordInProp = new ArrayList<Record>();
    static H1_TimeTool timeTool;

    public static void main(String[] args) {
        timeTool = new H1_TimeTool();
        initSystemInfo();
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (i == 0) {   // 第一个参数永远是  当前 shell的目录
                    ...
                    if (args[i].startsWith("clear")) {
                        isDeleteRecordOperation = true;
                        CUR_INPUT_DeleteParamStrList.add(args[i]);
                        continue;
                    }

                    // 以 day  month 开头的数据 那么执行 记录操作 把放到  H1.properties
                    if (args[i].startsWith("day[") || args[i].startsWith("month[") ||
                            args[i].startsWith("day_") || args[i].startsWith("month_") ||
                            args[i].startsWith("everymonth[") || args[i].startsWith("everyweek[")
                            || args[i].startsWith("everyyear[")) {
        ...
        if (isDeleteRecordOperation) {
            timeTool.deleteRecordOperation(CUR_INPUT_DeleteParamStrList);
    ...
    void deleteRecordOperation(ArrayList<String> clearStrList) {
...
//clearday0812
        ...
        for (int i = 0; i < clearStrList.size(); i++) {
            String clearItem = clearStrList.get(i);
            if (clearItem.toLowerCase().contains("clearall")) {
                clearAllRecord();
            } else if (clearItem.toLowerCase().contains("clearmonth")) {
                String clearMonthStr = clearItem.toLowerCase().replace("clearmonth", "");
                if (isNumeric(clearMonthStr)) {
                    int month = Integer.parseInt(clearMonthStr);
                    clearMonthRecord(month);
                }
            } else if (clearItem.toLowerCase().contains("clearweek")) {
                String clearWeekStr = clearItem.toLowerCase().replace("clearweek", "");

                if (clearWeekStr.contains("-")) {
                    String monthStr = clearWeekStr.split("-")[0];
                    String weekIndexStr = clearWeekStr.split("-")[1];
                    if (isNumeric(monthStr) && isNumeric(weekIndexStr)) {
                        int month = Integer.parseInt(monthStr);
                        int weekIndex = Integer.parseInt(weekIndexStr);
                        clearMontnWeekRecord(month, weekIndex);

                    }

                }
            } else if (clearItem.toLowerCase().contains("clearday")) {
                String clearDayStr = clearItem.toLowerCase().replace("clearday", "");

                while (clearDayStr.length() < 4) {
                    clearDayStr = "0" + clearDayStr;
                }
                if (isNumeric(clearDayStr)) {

                    String monthItemStr = clearDayStr.substring(0, 2);
                    String dayStr = clearDayStr.substring(2);
                    int month = Integer.parseInt(monthItemStr);
                    int day = Integer.parseInt(dayStr);
                    clearMontnDayRecord(month, day);
    ...
    void operationRecord(H1_TimeTool timeTool) {
        ...
        for (int i = 0; i < CUR_INPUT_ParamStrList.size(); i++) {
            ...
            if (inputItem.contains("everyyear")) {  // day[0607]  和 everyyear 相同的效果
                inputItem = inputItem.replace("everyyear", "day");
            }
            ...
            if (inputItem.toLowerCase().startsWith("day") && inputItem.contains("_")) {
                if (inputItem.toLowerCase().startsWith("day_")) {
                    ...
                    continue;
                } else if (inputItem.contains("[") && inputItem.contains("]")
                        && inputItem.indexOf("]") == inputItem.lastIndexOf("]")
                        && inputItem.indexOf("[") == inputItem.lastIndexOf("[")) {   // day[0402]_xxxx
                    String dateDesc = inputItem.split("_")[0];
                    dateDesc = dateDesc.replace("day", "");
                    dateDesc = dateDesc.replace("[", "");
                    dateDesc = dateDesc.replace("]", "");
                    if (!isNumeric(dateDesc)) {
                        continue;
                    }
                    while (dateDesc.length() < 4) {
                        dateDesc = "0" + dateDesc;
                    }

                    String twoCharMonthDesc = dateDesc.substring(0, 2);
                    String endDayDesc = dateDesc.substring(2);

                    int inputMonth = Integer.parseInt(twoCharMonthDesc);
                    int inputDay = Integer.parseInt(endDayDesc);

                    if (inputMonth <= 0 || inputMonth > 12) {
                        System.out.println("用户的输入参数: " + inputItem + "  无法解析为正常的月份请重新输入!");
                        continue;
                    }
                    if (inputDay <= 0 || inputDay > 32) {
                        System.out.println("用户的输入参数: " + inputItem + "  无法解析为正常的日份请重新输入!");
                        continue;
                    }
                    String dayDesc = inputItem.split("_")[1];
                    Record r = timeTool.new Record(inputMonth, inputDay, "", dayDesc, false, timestamp);
                    r.timeStamp = timestamp;
                    inputRecordList.add(r);
                ...
                if (inputItem.toLowerCase().startsWith("month_")) {
                    ...
                    continue;
                        ...
                        && inputItem.indexOf("[") == inputItem.lastIndexOf("[")) {   // day[0402]_xxxx
                    String dateDesc = inputItem.split("_")[0];
                    dateDesc = dateDesc.replace("month", "");
                    dateDesc = dateDesc.replace("[", "");
                    dateDesc = dateDesc.replace("]", "");
                    if (!isNumeric(dateDesc)) {
                        continue;
                    }
                    while (dateDesc.length() < 2) {
                        dateDesc = "0" + dateDesc;
                    }
                    ...
                    String monthDesc = inputItem.split("_")[1];
                    Record r = timeTool.new Record(inputMonth, 1, "", monthDesc, true, timestamp);
                    r.timeStamp = timestamp;
                    inputRecordList.add(r);

                }

            } else if (inputItem.toLowerCase().startsWith("everymonth") && inputItem.contains("_") &&
                    inputItem.contains("[") && inputItem.contains("]")
                    && inputItem.indexOf("]") == inputItem.lastIndexOf("]")
                    && inputItem.indexOf("[") == inputItem.lastIndexOf("[")) {

//                提示7:  ztime_H1.bat  everymonth[1]_AAAAAA             ## 记录一个循环每月日志 日志标识的时间是每个月的 1 号
//                提示7:  ztime_H1.bat  everymonth[31]_BBBBB              ## 记录一个循环每月日志 日志标识的时间是每个月的 31 号
//                提示7:  ztime_H1.bat  everyweek[1]_CCCCCC            ## 记录一个循环每周日志  该日志的时间为每周周一
//                提示7:  ztime_H1.bat  everyweek[7]DDDDDD            ## 记录一个循环每周日志  该日志的时间为每周周天
//                提示7:  ztime_H1.bat  everyyear[0207]_EEEEE            ## 记录一个每年的日志 该日志的时间0207    效果同 day[0207]_XXXXX


                String mEveryMonthDesc = inputItem.split("_")[0];
                mEveryMonthDesc = mEveryMonthDesc.replace("everymonth", "");
                mEveryMonthDesc = mEveryMonthDesc.replace("[", "");
                mEveryMonthDesc = mEveryMonthDesc.replace("]", "");
                if (!isNumeric(mEveryMonthDesc)) {
                    System.out.println("用户的输入参数: " + inputItem + "  无法解析为正常的日份请重新输入!");
                    continue;
                }
                int inputDay = Integer.parseInt(mEveryMonthDesc);

                if (inputDay <= 0 || inputDay > 32) {
                    System.out.println("用户的输入参数: " + inputItem + "  无法解析为正常的日份请重新输入!");
                    continue;
                }
                String everyMonthDayDesc = inputItem.split("_")[1];

                for (int j = 1; j <= 12; j++) {
                    long timestampItem = new Date().getTime();
                    if (checkMonthDayExist(j, inputDay)) {
                        Record r = timeTool.new Record(j, inputDay, "", everyMonthDayDesc, false, timestampItem);
                        inputRecordList.add(r);
                    }

                }

            } else if (inputItem.toLowerCase().startsWith("everyweek") && inputItem.contains("_") &&
                    inputItem.contains("[") && inputItem.contains("]")
                    && inputItem.indexOf("]") == inputItem.lastIndexOf("]")
                    && inputItem.indexOf("[") == inputItem.lastIndexOf("[")) {

//                everyweek[7]_DDDDDD
//                everyweek[11-7]_GGGGGG

                String mEveryWeekDesc = inputItem.split("_")[0];
                String mEveryWeekContent = inputItem.split("_")[1];
                mEveryWeekDesc = mEveryWeekDesc.replace("everyweek", "");
                mEveryWeekDesc = mEveryWeekDesc.replace("[", "");
                mEveryWeekDesc = mEveryWeekDesc.replace("]", "");

                int month = 0;
                int weeknum = 0;
                if (mEveryWeekDesc.contains("-")) {
                    String[] month_week = mEveryWeekDesc.split("-");
                    String monthStr = month_week[0];
                    String weekNumStr = month_week[1];
                    if (!isNumeric(monthStr) || !isNumeric(weekNumStr)) {
                        System.out.println("A 当前输入的 参数 不对 无法转为 周数 星期数  inputItem = " + inputItem + " mEveryWeekDesc = " + mEveryWeekDesc);
                        continue;
                    }
                    month = Integer.parseInt(monthStr);
                    weeknum = Integer.parseInt(weekNumStr);
                } else {
                    if (!isNumeric(mEveryWeekDesc)) {
                        System.out.println("B 当前输入的 参数 不对 无法转为 周数 星期数  inputItem = " + inputItem + " mEveryWeekDesc = " + mEveryWeekDesc);
                        continue;
                    }
                    weeknum = Integer.parseInt(mEveryWeekDesc);

                }

                ArrayList<Calendar> weekDate = calculAllWeekDate(month, weeknum);

                if (weekDate == null) {
                    System.out.println("C 当前输入的 参数 不对 无法转为 周数 星期数  inputItem = " + inputItem + " mEveryWeekDesc = " + mEveryWeekDesc);
                    continue;
                }
//                System.out.println(" month = "+month + " weeknum = "+ weeknum +"  周数 = "+ weekDate.size());

                for (int j = 0; j < weekDate.size(); j++) {
                    Calendar calItem = weekDate.get(j);
                    int monthItem = calItem.get(Calendar.MONTH) + 1;
                    int dayItem = calItem.get(Calendar.DAY_OF_MONTH);
                    long timestampItem = new Date().getTime();
                    Record r = timeTool.new Record(monthItem, dayItem, "", mEveryWeekContent, false, timestampItem);
                    inputRecordList.add(r);
                    System.out.println("增加Item_Index = " + j);
                }
...
// 输入格式1. 加减  ztime_H1.bat  18:30:17.560+5938s   2020-04-26_18:30:17.560-5938s  //1.没有年月日 默认今天   往后5938秒后的时间
        ...
        //            提示6:  ztime_H1.bat  day_XXXXXXXXXX            ## 记录一个日志 该日志的时间为今天
//            提示6:  ztime_H1.bat  month_YYYYYYYY            ## 记录一个月志 该日志的时间为今天
//            提示6:  ztime_H1.bat  day[0208]_XXXXXXXXXX            ## 记录一个日志 该日志的时间0208   默认为今年 0208
//            提示6:  ztime_H1.bat  month[5]_YYYYYYYY               ## 记录一个月志 该日志的时间为5月  默认今年为5月1日
//            提示7:  ztime_H1.bat  everymonth[1]_AAAAAA             ## 记录一个循环每月日志 日志标识的时间是每个月的 1 号
//            提示7:  ztime_H1.bat  everymonth[31]_BBBBB              ## 记录一个循环每月日志 日志标识的时间是每个月的 31 号
//            提示7:  ztime_H1.bat  everyweek[1]_CCCCCC            ## 记录一个循环每周日志  该日志的时间为每周周一
...
//
        ...
        boolean checkType6(String param) {
            ...
            // 1. 以  时间 参数为结尾
            endflag = param.startsWith("day_") || param.startsWith("day[") ||
                    param.startsWith("month_") || param.startsWith("month[") ||
                    param.startsWith("everymonth[") || param.startsWith("everyweek[") ||
                    param.startsWith("everymonth[");
                ...
                for (int j = 0; j < weekKeyList.size(); j++) {
                    ...
                    recordList.sort(new Comparator<Record>() {
                        ...
                        public int compare(Record o1, Record o2) {
                            if (o1.weekNum == o2.weekNum && o1.day == o2.day) {
                                return 0;
                            }
                            if (o1.weekNum == o2.weekNum && o1.day != o2.day) {
                    ...
                    if (recordItem == null) {
                        ...
                        String commonBlank_1 = "[";
                        String commonBlank_2 = "]";
                        if (isTodayRecord(recordItem)) {
                            commonBlank_1 = "=";
                            commonBlank_2 = "=";
                        }

                        sb.append(getPaddingString(commonBlank_1 + recordItem.weekNum + commonBlank_2 + recordItem.monthStr + recordItem.dayStr + " " + recordItem.dayDesc14Chinese, dayDescLength, " ", false) + "║");
                    }
...
//                System.out.println("weekDays.size = "+ weekDays.size());
                if(weekDays.size() == 0){
                    continue;
                }
                int beginDay = weekDays.get(0);
...
// value  就是 对应的需要显示在这个week下的记录了
    ...
    class Record {
        ...
        int year; //  如果有用户输入 那么就用 用户输入的  如果有没用户输入那么就用现在的
        int month;    //  记录的 月份
        ...
        int weekNum;   // 当日是星期几   1 2 3 4 5 6 7
        int weekIndex4Month;  // 当前记录是该月的第几个索引周  一般 0,1,2,3,4,5
...
//            static int ShowCalYearInt = 2020;
            ...
            weekNum = getWeekNum(year, month, day);

            weekIndex4Month = calWeekInMonthForDay(year, month, day);
        }
    ...
    static int getWeekNum(int yearParam, int monthParam, int dayParam) {
        int weekNum = 1;
...
//        beginMonthCalendar.set(beginMonthCalendar.DAY_OF_WEEK, beginMonthCalendar.MONDAY);
        beginMonthCalendar.set(Calendar.YEAR, yearParam); // 设置成年
        beginMonthCalendar.set(Calendar.MONTH, monthParam - 1); // 设置成年
        beginMonthCalendar.set(Calendar.DAY_OF_MONTH, dayParam); //
        beginMonthCalendar.set(Calendar.HOUR_OF_DAY, 0);
        beginMonthCalendar.set(Calendar.HOUR, 0);  //  时分秒 毫秒 都设置0
        beginMonthCalendar.set(Calendar.MINUTE, 0);  //  时分秒 毫秒 都设置0
        beginMonthCalendar.set(Calendar.SECOND, 0);  //  时分秒 毫秒 都设置0
        ...
        //0-周天  1-周一    6-周六
        weekNum = beginMonthCalendar.get(Calendar.DAY_OF_WEEK) - 1;

        if (weekNum == 0) {   // 周0转为 周七  以中国人的方式显示
            weekNum = 7;
        }
        return weekNum;
    }
...
//        int length = ("" + originStr).length();
    ...
    static boolean isTodayRecord(Record record) {
        boolean flag = false;
    ...
    // monthParam 为0  标识  是一整年的 星期 3   month为1--12 标识为 一个月的周三
    ArrayList<Calendar> calculAllWeekDate(int monthParam, int weekNumParam) {
        ArrayList<Calendar> weekDates = new ArrayList<Calendar>();

        int year = Now_YearInt;
        if (ShowCalYearInt != 2020) {
            year = ShowCalYearInt;
        } else {
            year = Now_YearInt;
        ...
        int daysInYear = calculRunYear(year);


        for (int i = 0; i <= daysInYear; i++) {
            Calendar calendar = calculDate(year, i);   // 计算一年的 第几天 返回 Date
            int monthItem = calendar.get(Calendar.MONTH) + 1;
            ...
            if (monthParam == 0) {
                if (weekNum == weekNumParam) {
                    weekDates.add(calendar);
                }
            } else {
                if (weekNum == weekNumParam && monthItem == monthParam) {
                    weekDates.add(calendar);
                }
            }
        }
    ...
    static Calendar calculDate(int yearParam, int dayParam) {
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.YEAR, yearParam);
        calendar.set(Calendar.DAY_OF_YEAR, dayParam);
        return calendar;
    }


    static boolean checkMonthDayExist(int mMonth, int mDay) {
        boolean falg = false;
        if (mMonth == 1) {
            falg = mDay >= 1 && mDay <= 31;
        } else if (mMonth == 2) {
            if (calculRunYear(ShowCalYearInt) == 366) {
                falg = mDay >= 1 && mDay <= 29;
            } else {
                falg = mDay >= 1 && mDay <= 28;
            }
    ...
    static int calWeekInMonthForDay(int year, int month, int day) {
        ...
        int weekIndex = 0;
        int beginXinqi = calculXinqiYiForMonthBegin(year, month);
        int monthDays = calculMonthDayCount(year, month);
        int virtualMonthIndex = 7 - beginXinqi + 1 + day;
        // 遇到 7 就是 一周
        weekIndex = (int) Math.floor(virtualMonthIndex / 7) + 1;

        if(weekIndex > 5){   // 一个月 默认 最大的周数是5  因为方便显示
            weekIndex = 5 ;
        }
