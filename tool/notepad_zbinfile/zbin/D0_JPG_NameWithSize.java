...
public class D0_JPG_NameWithSize {
    static String UserDir =  System.getProperty("user.dir");
    static String CurrentDir =  "";
    public static final ArrayList<File> jpgFileList_All = new ArrayList<File>();
    public static final ArrayList<File> jpgFileList_NeedFix = new ArrayList<File>();
    public static final ArrayList<File> jpgFileList_Fixed = new ArrayList<File>();
    public static Map<String,ArrayList<File>> nameMap = new HashMap<String,ArrayList<File>>();

    public static void main(String[] args) {


        String mjpgDirPath = null;
...
//          mjpgDirPath  = UserDir;          // 2.本地测试打开


        File curjpgDir;
        curjpgDir = new File(mjpgDirPath);
        if (mjpgDirPath != null && !mjpgDirPath.isEmpty() && (curjpgDir.exists()) && curjpgDir.isDirectory()) {
            CurrentDir = curjpgDir.getAbsolutePath();
        ...
        // 获取 当前路径下的 所有 jpg 文件
        System.out.println("当前文件夹路径:" + curjpgDir.getAbsolutePath());
        File[] curFileList = curjpgDir.listFiles();
        getjpgInCurrentDir(curFileList);
    ...
    static void addfixedFileItemAndUpdate( ) {
        // 获取新的名称
        if(jpgFileList_Fixed.size() > 0){
            for (int i = 0; i < jpgFileList_Fixed.size() ; i++) {
                File needFixFile = jpgFileList_Fixed.get(i);
        ...
        if(jpgFileList_NeedFix.size() > 0){
            for (int i = 0; i < jpgFileList_NeedFix.size() ; i++) {
                File needFixFile = jpgFileList_NeedFix.get(i);
                String fileName = needFixFile.getName();
...
//                                      }
    ...
    //
    static void getjpgInCurrentDir(File[] fileList) {
        ...
        for (File fileItem: fileList){
            ...
            if(fileName.endsWith(".jpg")){ //  如果文件以 .jpg 结尾
                jpgFileList_All.add(fileItem);
            }


        }

        if(jpgFileList_All.size() > 0){
            for (File jpgFileItem : jpgFileList_All ){

                isRuleNameStyle(jpgFileItem);
    ...
    static boolean isRuleNameStyle(File jpgFile) {
        ...
        if(isRuleFlag){
            ...
            jpgFileList_NeedFix.add(jpgFile);
        }
    ...
    static class FixNameFileItem{
        ...
        // 命名规则
// preType_sizeStr_typeSizeIndex_typeIndex.jpg
//land_0111x0111_002_011.jpg;
...
//20190824_21
            //   String timeStamp =
// // preType_sizeStr_typeSizeIndex_typeIndex+"时间戳?".jpg
            // typeSizeIndex 为三位   typeIndex为三位
            newName =type_size_identity+"_"+addForZeroStr(typeSizeIndex)+"_"+addForZeroStr(typeIndex)+".jpg";  // 定义名称
            newFileName = newName;
...
// 已经包含了前缀 land   取出 map
        ...
        void  update(){
            ...
            try {
                ...
                ImageIcon imageIcon = new ImageIcon(oldFile.getAbsolutePath());
                high = imageIcon.getIconHeight();
                width = imageIcon.getIconWidth();
            }catch (Exception e) {
                e.printStackTrace();
                System.out.println(" 解析jpg文件出现异常！ ");
            }
