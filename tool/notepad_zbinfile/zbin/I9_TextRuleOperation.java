...
//   把 一个 列表 保存到 prop ?
	...
	static String windows_FangDai_seperator_str = "╠═════════════════════════════════════════════════╣";
	class Shanghai_Home_PayMent_Calculate_Rule_62 extends Basic_Rule {

		Shanghai_Home_PayMent_Calculate_Rule_62(boolean mIsInputDirAsSearchPoint) {
			super(62);
			isInputDirAsSearchPoint = mIsInputDirAsSearchPoint;
		}

		Shanghai_Home_PayMent_Calculate_Rule_62() {
			super(62, false);
		...
		ArrayList<File> applyOperationRule(ArrayList<File> curFileList, HashMap<String, ArrayList<File>> subFileTypeMap,
			...
			for (int i = 0; i < curInputFileList.size(); i++) {
				...
				if(!isContain_FangDai_Input_Model(contentList)){
					// 在 I9_Temp_Text_File 中写入  房贷工具模型
				...
				calcul_fangdai_result(contentList);
...
//                NotePadOpenTargetFile(I9_Temp_Text_File.getAbsolutePath());
				System.out.println("rule_"+rule_index+" ->  检测到了输入的模板  将进行匹配生成对应的 房贷结果!  File=" + fileItem.getAbsolutePath());
			}
		...
		void calcul_fangdai_result(ArrayList<String> inputModelList){  //
			String line_seperator_str = windows_FangDai_seperator_str;
			if(CUR_OS_TYPE == OS_TYPE.MacOS){
				line_seperator_str =  macOS_FangDai_seperator_str;
			}
			int mInputModel_Count =  getInputModel_Count_InMacOS(inputModelList , line_seperator_str);

			if(mInputModel_Count == 0){
				System.out.println("当前从 文件识别到的输入模型的个数为 : "+ mInputModel_Count+" 个  请检查是否符合 输入模型 参数! ");
				return ;
			}

			System.out.println("当前从文件识别到的输入模型的个数为 : "+ mInputModel_Count+" 个   将开始识别参数!");

			// 去除 头部的 提示信息
			ArrayList<String> clearHeadTipInputModelList = clearHeadTipList(inputModelList,line_seperator_str);

			ArrayList<HomeDownPay> homeDownPayArrayList = calculAllInputModel_HomeDownPay(mInputModel_Count,clearHeadTipInputModelList);



			ArrayList<String> result_model_list  =  null;

			if(CUR_OS_TYPE == OS_TYPE.MacOS){
				result_model_list =     ShowPayMentInfo_In_MacOS_NotepadPP(homeDownPayArrayList);
			} else {
				result_model_list =     ShowPayMentInfo_In_Windows_NotepadPP(homeDownPayArrayList);

			}

			writeContentToFile(I9_Temp_Text_File, result_model_list);
			NotePadOpenTargetFile(I9_Temp_Text_File.getAbsolutePath());

			System.out.println("rule_"+rule_index+" -> 当前把房贷输入模型写入到 I9_Temp_Text_File 文件中 请参考  File=" + I9_Temp_Text_File.getAbsolutePath());
		...
		ArrayList<HomeDownPay> calculAllInputModel_HomeDownPay (int inputCount , ArrayList<String> rawInputList){
			ArrayList<HomeDownPay> mHomeDownPayList = new   ArrayList<HomeDownPay>();

			for (int i = 0; i < inputCount; i++) {
				boolean is_putongzhuzhai_input1 = false;
				boolean is_maijiaweiyi_input2 = false;
				boolean is_fiveyear_input3 = false;
				boolean is_buyer_firsthome_input4 = false;
				double double_homearea_input5 = 0 ;
				double double_sellerbuyprice_input6 = 0 ;
				double double_buyer_buy_home_allprice_input7 = 0 ;
				double double_middler_percent_input8 = 0 ;   // 中介费率
				double double_buyer_homepayment_percent_input9 = 0 ;   // 首付费率
				LoanType mLoanType_input10 = LoanType.GONGJIJIN ;   // 贷款方式
				double double_songjijinprice_in_allprice_percent_input11 = 0 ;   // 公积贷率
				double double_gongjijin_loan_price_input12 = 0 ;  // 公积贷额
				double double_gongjijin_loan_benefitrate_input13 = 0 ;  // 公积贷额利率
				int  int_gongjijin_loan_year_input14 = 0 ;  // 公积贷额 年限
				boolean  is_gongjijin_yuegong_same_permonth_input15 = false;  // 等额本息-true 等额本金-false

				double double_sahngye_price_in_allprice_percent_input16 = 0 ;   // 商业贷款贷率
				double double_shangye_loan_price_input17 = 0 ;  // 商业贷贷额
				double double_shangye_loan_benefitrate_input18 = 0 ;  // 商业贷额利率
				int  int_shangye_loan_year_input19 = 0 ;  // 商业贷 年限
				boolean  is_shangye_yuegong_same_permonth_input20 = false;  // 商业贷等额本息-true 等额本金-false



				for (int j = 0; j < rawInputList.size(); j++) {

					String oneLine = rawInputList.get(j);
					String[] inputStrArr =  oneLine.split("║║");

					System.out.println("split_length="+inputStrArr.length+" line["+j+"_"+rawInputList.size()+"]__"+oneLine);

					if(inputStrArr.length != inputCount){   // 当前无法解析的行
						System.out.println("当前无法解析的行 line_num= "+j+"   oneLine="+oneLine+"   inputStrArr.length ="+inputStrArr.length );
						continue;
					}
//                    for (int k = 0; k < inputStrArr.length ; k++) {
//                        String oneParam = inputStrArr[k].replace("║","").trim();
//                        System.out.println("split["+k+"_"+inputStrArr.length+"]__"+ oneParam);
//                    }

					String  match_index_param =  inputStrArr[i].replace("║","").trim();
					if(match_index_param == null || match_index_param.length() < 2){
						continue;
					}

					String index_order_str = match_index_param.substring(0,2);

					System.out.println("split["+i+"_"+inputStrArr.length+"]__"+ match_index_param+" XX["+index_order_str+"]");

					switch (index_order_str){

						case  "01":
							if(match_index_param.contains("非普通住宅")){   // ║01 普通住宅       非普通住宅
								is_putongzhuzhai_input1 = false;
							} else {
								is_putongzhuzhai_input1 = true;
							}
							break;

						case  "02":    // ║02 卖家唯一       卖家非唯一                     ║
							if(match_index_param.contains("卖家非唯一")){   // ║01 普通住宅       非普通住宅
								is_maijiaweiyi_input2 = false;
							} else {
								is_maijiaweiyi_input2 = true;
							}
							break;

						case  "03":   // ║03 房屋五年       房屋满两年                     ║
							if(match_index_param.contains("房屋五年")){   // ║01 普通住宅       非普通住宅
								is_fiveyear_input3 = true;
							} else {
								is_fiveyear_input3 = false;
							}
							break;

						case  "04":    // ║04 买家首套       买家第二套                     ║
							if(match_index_param.contains("买家首套")){   // ║01 普通住宅       非普通住宅
								is_buyer_firsthome_input4 = true;
							} else {
								is_buyer_firsthome_input4 = false;
							}
							break;

						case  "05":     // ║05 产证面积  【 】                              ║
							// ║05 产证面积 【 111.64 m2 】                     ║
							if(match_index_param.contains("产证面积")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("产证面积"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String are_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									are_raw_str = are_raw_str.replace("m2","").trim();

									if(isDoubleNumber(are_raw_str)){

										double_homearea_input5 =    Double.parseDouble(are_raw_str);
										System.out.println("******* double_homearea_input5 = "+ double_homearea_input5);
									}
							...
							break;
						case  "06":  // 06 卖家买入 【   1,300,000 】

							if(match_index_param.contains("卖家买入")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("卖家买入"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace(",","").trim();

									if(isDoubleNumber(fixed_raw_str)){

										double_sellerbuyprice_input6 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_sellerbuyprice_input6 = "+ double_sellerbuyprice_input6);
									}
							...
							break;
						case  "07":  // 07 合同总额 【   3,700,000 】
							if(match_index_param.contains("合同总额")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("合同总额"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace(",","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_buyer_buy_home_allprice_input7 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_buyer_buy_home_allprice_input7 = "+ double_buyer_buy_home_allprice_input7);
									}
							...
							break;
						case  "08":   // 08 中介费率 【  2   % 】
							if(match_index_param.contains("中介费率")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("中介费率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_middler_percent_input8 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_middler_percent_input8 = "+ double_middler_percent_input8);
									}
							...
							break;
						case  "09":   // 09 首付费率 【 35   % 】 首付金额【   1,295,000 】
							if(match_index_param.contains("首付费率")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("首付费率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_buyer_homepayment_percent_input9 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_buyer_homepayment_percent_input9 = "+ double_buyer_homepayment_percent_input9);
									}
							...
							break;
						case  "10":   // ║10 公积贷款   纯商贷款    混合贷款
							if(match_index_param.contains("公积贷款")){
								mLoanType_input10 = LoanType.GONGJIJIN ;
								System.out.println("******* mLoanType_input10 = 公积贷款 " );
							} else if(match_index_param.contains("纯商贷款")){
								mLoanType_input10 = LoanType.SHANGYE ;
								System.out.println("******* mLoanType_input10 = 纯商贷款 " );
							} else {
								mLoanType_input10 = LoanType.HUNHE ;
								System.out.println("******* mLoanType_input10 = 混合贷款 " );

							}
							break;
						case  "11":     // 11 公积贷率 【 65   % 】
							if(match_index_param.contains("公积贷率")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("公积贷率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_songjijinprice_in_allprice_percent_input11 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_songjijinprice_in_allprice_percent_input11 = "+ double_songjijinprice_in_allprice_percent_input11);
									}

								}
							}
							break;
						case  "12":   // 12 公积贷额 【   2,405,000 】
							if(match_index_param.contains("公积贷额")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("公积贷额"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace(",","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_gongjijin_loan_price_input12 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_gongjijin_loan_price_input12 = "+ double_gongjijin_loan_price_input12);
									}
							...
							break;
						case  "13":   // ║13 公积利率 【  3.25% 】
							if(match_index_param.contains("公积利率")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("公积利率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_gongjijin_loan_benefitrate_input13 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_gongjijin_loan_benefitrate_input13 = "+ double_gongjijin_loan_benefitrate_input13);
									}

								}
							}
							break;
						case  "14":   // ║14 公积年限 【 20   年限 】      [ 240 月 ]
							if(match_index_param.contains("公积年限")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("公积年限"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("年限","").trim();

									if(isDoubleNumber(fixed_raw_str)){

										int_gongjijin_loan_year_input14 =    Integer.parseInt(fixed_raw_str);
										System.out.println("******* int_gongjijin_loan_year_input14 = "+ int_gongjijin_loan_year_input14);
									}
						...
						case  "15": //  ║15 公积等额本息                                 ║
							...
							if(match_index_param.contains("公积等额本息")){
								is_gongjijin_yuegong_same_permonth_input15 = true ;
								System.out.println("******* is_gongjijin_yuegong_same_permonth_input15 = 公积等额本息 "  + is_gongjijin_yuegong_same_permonth_input15);
							} else {
								is_gongjijin_yuegong_same_permonth_input15 = false ;
								System.out.println("******* is_gongjijin_yuegong_same_permonth_input15  = 公积等额本金 " + is_gongjijin_yuegong_same_permonth_input15);
							}
							break;
						case  "16":  //  ║16 商业贷率 【  0   % 】                        ║
							if(match_index_param.contains("商业贷率")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("商业贷率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_sahngye_price_in_allprice_percent_input16 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_sahngye_price_in_allprice_percent_input16 = "+ double_sahngye_price_in_allprice_percent_input16);
									}
							...
							break;
						case  "17": // 17 商业贷额 【   1,110,000 】                   ║

							if(match_index_param.contains("商业贷额")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("商业贷额"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace(",","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_shangye_loan_price_input17 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_shangye_loan_price_input17 = "+ double_shangye_loan_price_input17);
									}
							...
							break;
						case  "18": // ║18 商贷利率 【  4.25% 】                        ║

							if(match_index_param.contains("商贷利率")){
								String blank_content = match_index_param.substring(match_index_param.indexOf("商贷利率"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("%","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										double_shangye_loan_benefitrate_input18 =    Double.parseDouble(fixed_raw_str);
										System.out.println("******* double_shangye_loan_benefitrate_input18 = "+ double_shangye_loan_benefitrate_input18);
									}
							...
							break;
						case  "19":  // ║19 商贷年限 【 20   年限 】      [ 240 月 ]      ║

							if(match_index_param.contains("商贷年限")){   // ║01 普通住宅       非普通住宅
								String blank_content = match_index_param.substring(match_index_param.indexOf("商贷年限"));
								if(blank_content.contains("【") && blank_content.contains("】") ){  // 获取 blank内的数据
									String fixed_raw_str =blank_content.substring(blank_content.indexOf("【")+1,blank_content.indexOf("】"));
									fixed_raw_str = fixed_raw_str.replace("年限","").trim();
									...
									if(isDoubleNumber(fixed_raw_str)){
										...
										int_shangye_loan_year_input19 =    Integer.parseInt(fixed_raw_str);
										System.out.println("******* int_shangye_loan_year_input19 = "+ int_shangye_loan_year_input19);
									}
						...
						case  "20":
							...
							if(match_index_param.contains("商贷等额本息")){
								is_shangye_yuegong_same_permonth_input20 = true ;
								System.out.println("******* is_shangye_yuegong_same_permonth_input20 = 公积等额本息 "  + is_shangye_yuegong_same_permonth_input20);
							} else {
								is_shangye_yuegong_same_permonth_input20 = false ;
								System.out.println("******* is_shangye_yuegong_same_permonth_input20  = 公积等额本金 " + is_shangye_yuegong_same_permonth_input20);
							}

							break;
						default:
							System.out.println("没有匹配到对应的 索引参数 index_order_str="+index_order_str);
							break;
					}
				...
				HomeDownPay mHomeDownPay = new HomeDownPay(is_putongzhuzhai_input1 , is_maijiaweiyi_input2 , is_fiveyear_input3, is_buyer_firsthome_input4, double_homearea_input5,
						double_sellerbuyprice_input6,double_buyer_buy_home_allprice_input7,double_middler_percent_input8,double_buyer_homepayment_percent_input9,mLoanType_input10,
						double_songjijinprice_in_allprice_percent_input11,double_gongjijin_loan_price_input12,double_gongjijin_loan_benefitrate_input13,int_gongjijin_loan_year_input14,is_gongjijin_yuegong_same_permonth_input15,
						double_sahngye_price_in_allprice_percent_input16,double_shangye_loan_price_input17,double_shangye_loan_benefitrate_input18,int_shangye_loan_year_input19,is_shangye_yuegong_same_permonth_input20);

				mHomeDownPayList.add(mHomeDownPay);
			}

			return mHomeDownPayList;

		}


		ArrayList<String> clearHeadTipList ( ArrayList<String> rawInputList , String  seperatorLineStr ){
			ArrayList<String> clearheadList  = new   ArrayList<String>();

			boolean begin_flag = false;

			for (int i = 0; i < rawInputList.size(); i++) {
				String oneLine =  rawInputList.get(i);

				if(oneLine.startsWith(seperatorLineStr)){
					begin_flag = true;
					continue;
				}
				...
				if(begin_flag){
					if(oneLine.startsWith(seperatorLineStr)){
						begin_flag = false;
						return clearheadList;
					}
					clearheadList.add(oneLine);
				}
		...
		int  getInputModel_Count_InMacOS(ArrayList<String> inputModelList , String seperatorLineStr){
			int count = 0 ;
			for (int i = 0; i < inputModelList.size(); i++) {
				String oneLine= inputModelList.get(i);

				if(oneLine.startsWith(seperatorLineStr)){
					int matchCount =     oneLine.trim().length()/seperatorLineStr.length();
		...
		void init_input_model(){
			ArrayList<HomeDownPay> mHomeDownPayList_1 = new ArrayList<HomeDownPay>();
			HomeDownPay mHomeDownPay_0 = new HomeDownPay(false,false,false,false,
					80,1000000 , 3000000,2,35 ,
					LoanType.HUNHE
					,0,1200000 ,3.25,20,true
					,0,750000 ,4.9,20,false );

			HomeDownPay mHomeDownPay_1 = new HomeDownPay(false,false,true,true,
					90,1500000 , 3200000,2,35 ,
					LoanType.HUNHE
					,35,0 ,3.25,20,true
					,30,0 ,4.5,30,false );

			HomeDownPay mHomeDownPay_2 = new HomeDownPay(true,true,true,true,
					90,1500000 , 3500000,2,35 ,
					LoanType.HUNHE
					,35,0 ,3.25,20,true
					,30,0 ,4.5,30,false );

			HomeDownPay mHomeDownPay_3 = new HomeDownPay(true,true,false,false,
					120,1800000 , 3600000,2,35 ,
					LoanType.HUNHE
					,0,1200000 ,3.25,15,true
					,0,1140000 ,4.5,20,true );

			mHomeDownPayList_1.add(mHomeDownPay_0);
			mHomeDownPayList_1.add(mHomeDownPay_1);
			mHomeDownPayList_1.add(mHomeDownPay_2);
			mHomeDownPayList_1.add(mHomeDownPay_3);


			ArrayList<String> input_model_list  =  null;

			if(CUR_OS_TYPE == OS_TYPE.MacOS){
				input_model_list =     ShowInputModel_In_MacOS_NotepadPP(mHomeDownPayList_1);
			} else {
				input_model_list =     ShowInputModel_In_WinOS_NotepadPP(mHomeDownPayList_1);

			}

			writeContentToFile(I9_Temp_Text_File, input_model_list);
			NotePadOpenTargetFile(I9_Temp_Text_File.getAbsolutePath());
		...
		public   ArrayList<String> ShowPayMentInfo_In_Windows_NotepadPP(ArrayList<HomeDownPay> mDownPayList ){

			HomeDownPay   mHomeDownPayList_1   = mDownPayList.get(0);
			ArrayList<String> out_result_List = new  ArrayList<String>();
			System.out.println("___________________________ Windows Notepad++ 房贷结算结果 ___________________________");

			out_result_List.add("___________________________ Windows Notepad++ 房贷结算结果  ___________________________");
			for (int i = 0; i < mHomeDownPayList_1.mtableStr_WinOS_Notepadxx_Output_List.size(); i++) {
				StringBuilder  mLogSBLine = new StringBuilder();
				for (int j = 0; j < mDownPayList.size(); j++) {
					String mLogLine =    mDownPayList.get(j).mtableStr_WinOS_Notepadxx_Output_List.get(i);


					mLogLine = mLogLine.replaceAll("\n","");

					mLogSBLine.append(mLogLine);


				}
				out_result_List.add(mLogSBLine.toString());
				System.out.println(mLogSBLine);
		...
		public   ArrayList<String> ShowPayMentInfo_In_MacOS_NotepadPP(ArrayList<HomeDownPay> mDownPayList ){

			HomeDownPay   mHomeDownPayList_1   = mDownPayList.get(0);
			ArrayList<String> out_result_List = new  ArrayList<String>();
			System.out.println("___________________________ MacOS Notepad++ 房贷结算结果 style ___________________________");
			out_result_List.add("___________________________ MacOS Notepad++ 房贷结算结果 style ___________________________");
			for (int i = 0; i < mHomeDownPayList_1.mtableStr_MacOS_Notepadxx_Output_List.size(); i++) {
				StringBuilder  mLogSBLine = new StringBuilder();
				for (int j = 0; j < mDownPayList.size(); j++) {
					String mLogLine =    mDownPayList.get(j).mtableStr_MacOS_Notepadxx_Output_List.get(i);
		...
		public   ArrayList<String> ShowInputModel_In_WinOS_NotepadPP(ArrayList<HomeDownPay> mDownPayList ){
			ArrayList<String> InputModel_List = new  ArrayList<String>();
			HomeDownPay   mHomeDownPayList_1   = mDownPayList.get(0);
			System.out.println ("___________________________ Windows Notepad++ InputModel ___________________________");
			InputModel_List.add("___________________________ Windows Notepad++ InputModel ___________________________");
			for (int i = 0; i < mHomeDownPayList_1.mtableStr_WinOS_Notepadxx_Input_ModelList.size(); i++) {
				StringBuilder  mLogSBLine = new StringBuilder();
				for (int j = 0; j < mDownPayList.size(); j++) {
					String mLogLine =    mDownPayList.get(j).mtableStr_WinOS_Notepadxx_Input_ModelList.get(i);
				...
				InputModel_List.add(mLogSBLine.toString());
				System.out.println(mLogSBLine);
			...
			return InputModel_List;
		}


		public   ArrayList<String> ShowInputModel_In_MacOS_NotepadPP(ArrayList<HomeDownPay> mDownPayList ){
			ArrayList<String> InputModel_List = new  ArrayList<String>();
			HomeDownPay   mHomeDownPayList_1   = mDownPayList.get(0);
			System.out.println("___________________________ MacOS Notepad-- InputModel ___________________________");
			InputModel_List.add("___________________________ MacOS Notepad-- InputModel ___________________________");
			for (int i = 0; i < mHomeDownPayList_1.mtableStr_MacOS_Notepadxx_Input_ModelList.size(); i++) {
				StringBuilder  mLogSBLine = new StringBuilder();
				for (int j = 0; j < mDownPayList.size(); j++) {
					String mLogLine =    mDownPayList.get(j).mtableStr_MacOS_Notepadxx_Output_List.get(i);
		...
		boolean isContain_FangDai_Input_Model(ArrayList<String> rawContentList){

			// 在 同一行  包含 这三个 tag 那么 可能 就是 输入的模板了
			String tag_1 = "公积贷款";
			String tag_2 = "纯商贷款";
			String tag_3 = "混合贷款";
			...
			boolean flag_tag = false;
			boolean flag_seperator = false;
			for (int i = 0; i < rawContentList.size(); i++) {
				String lineStr = rawContentList.get(i);

				if(lineStr.contains(tag_1) && lineStr.contains(tag_2) && lineStr.contains(tag_3)){

					flag_tag =  true;
				}


				if(lineStr.contains(line_seperator_str)){
			...
			return flag_tag && flag_seperator;
		}

		@Override
		String simpleDesc() {
			return " 读取当前txt输入房贷计算器的模板进行计算(没有模板那么将会打印出模板)输出房贷信息列表";
		}
	...
	public static class HomeDownPay {
...
// ###########################   需要得到的输入 ##########################
		...
		HomeDownPay(boolean xseller_home_isputong_flag ,boolean xseller_first_home_flag , boolean xseller_home_fiveyear_flag , boolean xbuyer_first_home_flag
			...
			mtableStr_MacOS_Notepadxx_Output_List = new ArrayList<String>();
			mtableStr_MacOS_Notepadxx_Input_ModelList =  new ArrayList<String>();
			MacOS_Notepadxx_initTableStrLit();


			mtableStr_WinOS_Notepadxx_Output_List = new ArrayList<String>();
			mtableStr_WinOS_Notepadxx_Input_ModelList =  new ArrayList<String>();
			WinOS_Notepadxx_initTableStrLit();
...
/*        mtableStr_MacOS_Notepad_List = new ArrayList<String>();
		...
		public void  WinOS_Notepadxx_initTableStrLit(){
			ArrayList<String> logArr = new ArrayList<String>();

			String head_tip_line = "╔═════════════════════════════════════════════════╗";
			String head_tip_1 =  "║01 普通住宅       非普通住宅                     ║";
			String head_tip_2 =  "║02 卖家唯一       卖家非唯一                     ║";
			String head_tip_3 =  "║03 房屋五年       房屋满两年                     ║";
			String head_tip_4 =  "║04 买家首套       买家第二套                     ║";
			String head_tip_5 =  "║05 产证面积  【 】                               ║";
			String head_tip_6 =  "║06 卖家买入  【 】                               ║";
			String head_tip_7 =  "║07 合同总额  【 】                               ║";
			String head_tip_8 =  "║08 中介费率  【 】       中介金额【 】           ║";
			String head_tip_9 =  "║09 首付费率  【 】       首付金额【 】           ║";
			String head_tip_10 = "║10 公积贷款   纯商贷款    混合贷款               ║";
			String head_tip_11 = "║11 公积贷率  【 】                               ║";
			String head_tip_12 = "║12 公积贷额  【 】                               ║";
			String head_tip_13 = "║13 公积利率  【 】                               ║";
			String head_tip_14 = "║14 公积年限  【    年限 】      [     月]        ║";
			String head_tip_15 = "║15 公积等额本息    公积等额本金                  ║";
			String head_tip_16 = "║16 商业贷率  【 】                               ║";
			String head_tip_17 = "║17 商业贷额  【 】                               ║";
			String head_tip_18 = "║18 商贷利率  【 】                               ║";
			String head_tip_19 = "║19 商贷年限  【    年限 】      [     月]        ║";
			String head_tip_20 = "║20 商贷等额本息    商贷等额本金                  ║";

			logArr.add(head_tip_line);
			logArr.add(head_tip_1);
			logArr.add(head_tip_2);
			logArr.add(head_tip_3);
			logArr.add(head_tip_4);
			logArr.add(head_tip_5);
			logArr.add(head_tip_6);
			logArr.add(head_tip_7);
			logArr.add(head_tip_8);
			logArr.add(head_tip_9);
			logArr.add(head_tip_10);
			logArr.add(head_tip_11);
			logArr.add(head_tip_12);
			logArr.add(head_tip_13);
			logArr.add(head_tip_14);
			logArr.add(head_tip_15);
			logArr.add(head_tip_16);
			logArr.add(head_tip_17);
			logArr.add(head_tip_18);
			logArr.add(head_tip_19);
			logArr.add(head_tip_20);


			String input_tip_line = "╠═════════════════════════════════════════════════╣";
			String input_tip_1 =  seller_home_isputong_flag?   "║01 【普通住宅】                                  ║":   "║01                非普通住宅                     ║";
			String input_tip_2    =    seller_first_home_flag?   "║01 【卖家唯一】                                  ║": "║02                卖家非唯一                     ║";
			String input_tip_3 =    seller_home_fiveyear_flag?"║03 【房屋五年】                                  ║": "║03                卖家满两年                     ║";
			String input_tip_4 =    buyer_first_home_flag?        "║04 【买家首套】                                  ║": "║04                买家第二套                     ║";
			String input_tip_5 =  "║05 产证面积 【 "+getAreaPercentStr(home_area)+" m2 】                      ║";
			String input_tip_6 =  "║06 卖家买入 【 "+getPriceStr(seller_buyhome_price)+" 】                    ║";
			String input_tip_7 =  "║07 合同总额 【 "+getPriceStr(buyer_buyhome_seller_sellhome_price)+" 】                    ║";
			String input_tip_8 =  "║08 中介费率 【 "+getPercentStr(input_buyer_buyhome_zhongjie_percent *0.01)+" 】 中介金额[ "+ getPriceStr(input_buyer_buyhome_zhongjie_price)+" ] ║";
			String input_tip_9 =  "║09 首付费率 【 "+getPercentStr(buyer_buyhome_downpayment_percent*0.01)+" 】 首付金额[ "+getPriceStr(buyer_buyhome_downpayment_price)+" ] ║";
			String input_tip_10 = "║10 【公积贷款】                                   ║";
			switch (mLoanEntry.mLoanType){
				case GONGJIJIN:
					input_tip_10 = "║10 【公积贷款】                                  ║";
					break;
				case SHANGYE:
					input_tip_10 = "║10          【纯商贷款】                         ║";
					break;
				case HUNHE:
					input_tip_10 = "║10                     【混合贷款】              ║";
					break;
			}



			String input_tip_11 =  "║11 公积贷率 【 "+getPercentStr(mLoanEntry.getGongJiJin_LoanPrice_In_AllPrice())+" 】                         ║";
			String input_tip_12 =  "║12 公积贷额 【 "+getPriceStr(mLoanEntry.loanentry_gongjijin_loan_price)+" 】                    ║";
			String input_tip_13 =  "║13 公积利率 【 "+getPercentStr(mLoanEntry.loanentry_gongjijin_loan_benefit_rate * 0.01 )+" 】                         ║";

			String input_tip_14 =  "║14 公积年限 【 "+getTwoIntStr(mLoanEntry.loanentry_gongjijin_loan_yearcount)+"   年限 】      [ "+getThreeIntStr(mLoanEntry.loanentry_gongjijin_loan_yearcount*12)+" 月 ]      ║";
			String input_tip_15 =  "║15 公积等额本息                                  ║";
			if(mLoanEntry.loanentry_gongjijin_loan_is_samepay_anymonth){
				input_tip_15 =  "║15 公积等额本息                                  ║";
			} else {
				input_tip_15 =  "║15                公积等额本金                   ║";
			}



			String input_tip_16 =  "║16 商业贷率 【 "+getPercentStr(mLoanEntry.getShangYe_LoanPrice_In_AllPrice())+" 】                         ║";
			String input_tip_17 =  "║17 商业贷额 【 "+getPriceStr(mLoanEntry.loanentry_shangye_loan_price)+" 】                    ║";

			String input_tip_18 =  "║18 商贷利率 【 "+getPercentStr(mLoanEntry.loanentry_shangye_loan_benefit_rate * 0.01 )+" 】                         ║";
			String input_tip_19 =  "║19 商贷年限 【 "+getTwoIntStr(mLoanEntry.loanentry_shangye_loan_yearcount)+"   年限 】      [ "+getThreeIntStr(mLoanEntry.loanentry_shangye_loan_yearcount*12)+" 月 ]      ║";
			String input_tip_20 =  "║20 商贷等额本息                                  ║";
			if(mLoanEntry.loanentry_shangye_loan_is_samepay_anymonth){
				input_tip_20 =  "║20 商贷等额本息                                  ║";
			} else {
				input_tip_20 =  "║20                  商贷等额本金                 ║";
			}
			logArr.add(input_tip_line);
			logArr.add(input_tip_1);
			logArr.add(input_tip_2);
			logArr.add(input_tip_3);
			logArr.add(input_tip_4);
			logArr.add(input_tip_5);
			logArr.add(input_tip_6);
			logArr.add(input_tip_7);
			logArr.add(input_tip_8);
			logArr.add(input_tip_9);
			logArr.add(input_tip_10);
			logArr.add(input_tip_11);
			logArr.add(input_tip_12);
			logArr.add(input_tip_13);
			logArr.add(input_tip_14);
			logArr.add(input_tip_15);
			logArr.add(input_tip_16);
			logArr.add(input_tip_17);
			logArr.add(input_tip_18);
			logArr.add(input_tip_19);
			logArr.add(input_tip_20);

			String output_tip = "╠═════════════════════════════════════════════════╣";
			WinOS_Notepadxx_init_input_model_list(logArr , output_tip);
			String output_tip_1 = "║01 增值税率 【 "+getPercentStr(buyer_buyhome_zengzhishui_percent)+" 】增值税额【 "+getPriceStr(buyer_buyhome_zengzhishui_price)+" 】║";
			String output_tip_2 = "║02 附加税率 【 "+getPercentStr(buyer_buyhome_fujiashui_percent)+" 】附加税额【 "+getPriceStr(buyer_buyhome_fujiashui_price)+" 】║";
			String output_tip_3 = "║03 个税费率 【 "+getPercentStr(buyer_buyhome_geshui_percent)+" 】个税金额【 "+getPriceStr(buyer_buyhome_geshui_price)+" 】║";
			String output_tip_4 = "║04 契税费率 【 "+getPercentStr(buyer_buyhome_qishui_percent)+" 】契税金额【 "+getPriceStr(buyer_buyhome_qishui_price)+" 】║";
			String output_tip_5 = "║05 中介费率 【 "+getPercentStr(input_buyer_buyhome_zhongjie_percent *0.01)+" 】中介金额【 "+ getPriceStr(input_buyer_buyhome_zhongjie_price) +" 】║";
			String output_tip_6 = "║06 首付费率 【 "+getPercentStr(buyer_buyhome_downpayment_percent*0.01)+" 】首付金额【 "+getPriceStr(buyer_buyhome_downpayment_price)+" 】║";
			String buyer_all_tax_price_tip=   "║___总税费额_____________________【 "+getPriceStr(buyer_all_tax_price)+" 】║";
			String buyer_first_buyhome_price_tip=   "║___总首付额_____________________【 "+getPriceStr(buyer_first_buyhome_price)+" 】║";
			String output_tip_7 = mLoanEntry.isHasGongJiJin_Loan()?
					"║07 公积贷额 【 "+getPriceStr(mLoanEntry.loanentry_gongjijin_loan_price)+" 】   ["+getTwoPointTwoPercentStr(mLoanEntry.loanentry_gongjijin_loan_benefit_rate * 0.01).replace(" ","")+"]          ║"
					:
					"║07                                               ║";
			String output_tip_8 =mLoanEntry.isHasShangYe_Loan()?
					"║08 商业贷额 【 "+getPriceStr(mLoanEntry.loanentry_shangye_loan_price)+" 】   ["+getTwoPointTwoPercentStr(mLoanEntry.loanentry_shangye_loan_benefit_rate * 0.01).replace(" ","")+"]          ║"
					:
					"║08                                               ║";
			String output_tip_9 =             "║09 总贷额额 【 "+getPriceStr(mLoanEntry.getAll_loan_price())+" 】                    ║";

			String output_tip_10 =             "║10 首付金额 【 "+getPriceStr(buyer_buyhome_downpayment_price)+" 】                    ║";
			String output_tip_11 =             "║11 估房总价 【 "+getPriceStr(buyer_buyhome_downpayment_price + mLoanEntry.getAll_loan_price())+" 】                    ║";




			String output_tip_12 =mLoanEntry.isHasGongJiJin_Loan()?
					"║12 公积月供 【 "+getPriceStr(mLoanEntry.getGongJiJin_YueGong())+" 】逐_"+getThreeIntStr(mLoanEntry.loanentry_gongjijin_loan_yearcount*12)+"_月减[ "+ getDoublePoint_FiveStr(mLoanEntry.getGongJiin_YueGong_DownValue_PerMonth())+" ]║"
					:
					"║12                                               ║";
			String output_tip_13 =mLoanEntry.isHasShangYe_Loan()?
					"║13 商贷月供 【 "+getPriceStr(mLoanEntry.getShangYe_YueGong())+" 】逐_"+getThreeIntStr(mLoanEntry.loanentry_shangye_loan_yearcount*12)+"_月减[ "+ getDoublePoint_FiveStr(mLoanEntry.getShangYe_YueGong_DownValue_PerMonth())+" ]║"
					:
					"║13                                               ║";
			...
			String output_tip_zong_yuegong =  "║___总贷月供_【 "+getPriceStr(mLoanEntry.get_all_yuegong())+" 】逐_"+getThreeIntStr(mLoanEntry.get_min_yearcount()*12)+"_月减[ "+ getDoublePoint_FiveStr(mLoanEntry.get_all_yuegong_downvalue_permonth())+" ]║";

			String output_tip_14 =mLoanEntry.isHasGongJiJin_Loan()?
					"║14 公积总利 【 "+getPriceStr(mLoanEntry.get_gongjijin_zonglixi())+" 】                    ║"
					:
					"║14                                               ║";


			String output_tip_15 =  mLoanEntry.isHasShangYe_Loan() ?
					"║15 商贷总利 【 "+getPriceStr(mLoanEntry.get_shangye_zonglixi())+" 】                    ║"
					:
					"║15                                               ║";


			String output_tip_zong_lixi =     "║___总还利息_【 "+getPriceStr(mLoanEntry.get_zonglixi())+" 】                    ║";
			String output_tip_16 =mLoanEntry.isHasGongJiJin_Loan()?
					"║16 公积总还 【 "+getPriceStr(mLoanEntry.get_gongjijin_zonghuang())+" 】                    ║"
					:
					"║16                                               ║";

			String output_tip_17 =  mLoanEntry.isHasShangYe_Loan() ?
					"║17 商贷总还 【 "+getPriceStr(mLoanEntry.get_shangye_zonghuang())+" 】                    ║"
					:
					"║17                                               ║";
			String output_tip_zong_payment =  "║___总还款额_【 "+getPriceStr(mLoanEntry.get_zonghuang())+" 】                    ║";
			String end_head_tip= "╚═════════════════════════════════════════════════╝";

			logArr.add(output_tip);
			logArr.add(output_tip_1);
			logArr.add(output_tip_2);
			logArr.add(output_tip_3);
			logArr.add(output_tip_4);
			logArr.add(output_tip_5);
			logArr.add(output_tip_6);
			logArr.add(buyer_all_tax_price_tip);
			logArr.add(buyer_first_buyhome_price_tip);
			logArr.add(output_tip_7);
			logArr.add(output_tip_8);


			logArr.add(output_tip_9);
			logArr.add(output_tip_10);
			logArr.add(output_tip_11);
			logArr.add(output_tip_12);

			logArr.add(output_tip_13);
			logArr.add(output_tip_zong_yuegong);
			logArr.add(output_tip_14);
			logArr.add(output_tip_15);
			logArr.add(output_tip_zong_lixi);
			logArr.add(output_tip_16);
			logArr.add(output_tip_17);
			logArr.add(output_tip_zong_payment);
			logArr.add(end_head_tip);


			StringBuilder sb = new StringBuilder();
			mtableStr_WinOS_Notepadxx_Output_List.clear();
			for (int i = 0; i < logArr.size(); i++) {
				String endPrintLine =  logArr.get(i)+"\n";

				sb.append(endPrintLine);
				mtableStr_WinOS_Notepadxx_Output_List.add(endPrintLine);
			}

		}
		public void MacOS_Notepadxx_initTableStrLit() {
			ArrayList<String> logArr = new ArrayList<String>();
			...
			String input_tip_line = "╠═══════════════════════════════════════╣";
			String input_tip_1 =  seller_home_isputong_flag?   "║01 【普通住宅】                                 ║": "║01              ※非普通住宅                    ║";
			String input_tip_2    =    seller_first_home_flag?   "║02 【卖家唯一】                                 ║": "║02              ※卖家非唯一                    ║";
			String input_tip_3 =    seller_home_fiveyear_flag?"║03 【房屋五年】                                 ║": "║03              ※房屋满两年                    ║";
			String input_tip_4 =    buyer_first_home_flag?        "║04 【买家首套】                                 ║": "║04              ※买家第二套                    ║";
			String input_tip_5 =  "║05 产证面积 【 "+getAreaPercentStr(home_area)+" m2 】                     ║";
			String input_tip_6 =  "║06 卖家买入 【 "+getPriceStr(seller_buyhome_price)+" 】                   ║";
			String input_tip_7 =  "║07 合同总额 【 "+getPriceStr(buyer_buyhome_seller_sellhome_price)+" 】                   ║";
			...
			String output_tip = "╠═══════════════════════════════════════╣";
			MacOS_Notepadxx_init_input_model_list(logArr , output_tip);
			String output_tip_1 = "║01 增值税率 【 "+getPercentStr(buyer_buyhome_zengzhishui_percent)+" 】 增值税额【 "+getPriceStr(buyer_buyhome_zengzhishui_price)+" 】║";
			String output_tip_2 = "║02 附加税率 【 "+getPercentStr(buyer_buyhome_fujiashui_percent)+" 】 附加税额【 "+getPriceStr(buyer_buyhome_fujiashui_price)+" 】║";
			String output_tip_3 = "║03 个税费率 【 "+getPercentStr(buyer_buyhome_geshui_percent)+" 】 个税金额【 "+getPriceStr(buyer_buyhome_geshui_price)+" 】║";
			String output_tip_4 = "║04 契税费率 【 "+getPercentStr(buyer_buyhome_qishui_percent)+" 】 契税金额【 "+getPriceStr(buyer_buyhome_qishui_price)+" 】║";
			String output_tip_5 = "║05 中介费率 【 "+getPercentStr(input_buyer_buyhome_zhongjie_percent *0.01)+" 】 中介金额【 "+ getPriceStr(input_buyer_buyhome_zhongjie_price) +" 】║";
			String output_tip_6 = "║06 首付费率 【 "+getPercentStr(buyer_buyhome_downpayment_percent*0.01)+" 】 首付金额【 "+getPriceStr(buyer_buyhome_downpayment_price)+" 】║";
			String buyer_all_tax_price_tip=   "║___总税费额____________________【 "+getPriceStr(buyer_all_tax_price)+" 】║";
			String buyer_first_buyhome_price_tip=   "║___总首付额____________________【 "+getPriceStr(buyer_first_buyhome_price)+" 】║";
			String output_tip_7 = mLoanEntry.isHasGongJiJin_Loan()?
					"║07 公积贷额 【 "+getPriceStr(mLoanEntry.loanentry_gongjijin_loan_price)+" 】   ["+getPercentStr(mLoanEntry.loanentry_gongjijin_loan_benefit_rate * 0.01).replace(" ","")+"]         ║"
					:
					"║07                                            ║";
			String output_tip_8 =mLoanEntry.isHasShangYe_Loan()?
					"║08 商业贷额 【 "+getPriceStr(mLoanEntry.loanentry_shangye_loan_price)+" 】   ["+getPercentStr(mLoanEntry.loanentry_shangye_loan_benefit_rate * 0.01).replace(" ","")+"]         ║"
					:
					"║08                                            ║";
			String output_tip_9 =             "║09 总贷额额 【 "+getPriceStr(mLoanEntry.getAll_loan_price())+" 】                   ║";
			...
			StringBuilder sb = new StringBuilder();
			mtableStr_MacOS_Notepadxx_Output_List.clear();
			for (int i = 0; i < logArr.size(); i++) {
				String endPrintLine =  logArr.get(i)+"\n";

				sb.append(endPrintLine);
				mtableStr_MacOS_Notepadxx_Output_List.add(endPrintLine);
			}
		...
		public String toString() {
			String head_tip_line = "╔═══════════════════════════════════════╗";
			String head_tip_1 = "1.普通住宅        非普通住宅                     ";
			String head_tip_2 = "2.卖家唯一        卖家非唯一                     ";
			String head_tip_3 = "3.房屋五年        房屋满两年                     ";
			String head_tip_4 = "4.买家首套        买家第二套                     ";
			String head_tip_5 = "5.产证面积  【 】                               ";
			String head_tip_6 = "6.卖家买入  【 】                               ";
			String head_tip_7 = "7.合同总额  【 】                               ";
			String head_tip_8 = "8.中介费率  【 】  中介金额【 】                  ";
			String head_tip_9 = "9.首付费率  【 】  首付金额【 】                  ";


			String input_tip = "╠═══════════════════════════════════════╣";
			String seller_home_isputong_flag_tip = seller_home_isputong_flag?   "1.【普通住宅】                                  ": "1.              ※非普通住宅                    ";
			String seller_first_home_flag_tip    =    seller_first_home_flag?   "2.【卖家唯一】                                  ": "2.              ※卖家非唯一                    ";
			String seller_home_fiveyear_flag_tip =    seller_home_fiveyear_flag?"3.【房屋五年】                                  ": "3.              ※房屋满两年                    ";
			String buyer_first_home_flag_tip =    buyer_first_home_flag?        "4.【买家首套】                                  ": "4.              ※买家第二套                    ";
			String home_area_tip =                                              "5.产证面积 【 "+getAreaPercentStr(home_area)+" m2 】                      ";
			String seller_buyhome_price_tip =                                   "6.卖家买入 【 "+getPriceStr(seller_buyhome_price)+" 】                    ";
			String buyer_buyhome_seller_sellhome_price_tip =                    "7.合同总额 【 "+getPriceStr(buyer_buyhome_seller_sellhome_price)+" 】                    ";
			String buyer_buyhome_zhongjie_tip =                                 "8.中介费率 【 "+getPercentStr(input_buyer_buyhome_zhongjie_percent *0.01)+" 】 中介金额【 "+ getPriceStr(input_buyer_buyhome_zhongjie_price)+" 】 ";
			String buyer_buyhome_downpayment_tip =                              "9.首付费率 【 "+getPercentStr(buyer_buyhome_downpayment_percent*0.01)+" 】 首付金额【 "+getPriceStr(buyer_buyhome_downpayment_price)+" 】 ";

			String output_tip = "╠═══════════════════════════════════════╣";
			String buyer_buyhome_zengzhishui_tip =  "1.增值税率 【 "+getPercentStr(buyer_buyhome_zengzhishui_percent)+" 】 增值税额【 "+getPriceStr(buyer_buyhome_zengzhishui_price)+" 】 ";
			String buyer_buyhome_fujiashui_tip =    "2.附加税率 【 "+getPercentStr(buyer_buyhome_fujiashui_percent)+" 】 附加税额【 "+getPriceStr(buyer_buyhome_fujiashui_price)+" 】 ";
			String buyer_buyhome_geshui_tip =       "3.个税费率 【 "+getPercentStr(buyer_buyhome_geshui_percent)+" 】 个税金额【 "+getPriceStr(buyer_buyhome_geshui_price)+" 】 ";
			String buyer_buyhome_qishui_tip =       "4.契税费率 【 "+getPercentStr(buyer_buyhome_qishui_percent)+" 】 契税金额【 "+getPriceStr(buyer_buyhome_qishui_price)+" 】 ";
			String buyer_buyhome_zhongjie_outtip =  "5.中介费率 【 "+getPercentStr(input_buyer_buyhome_zhongjie_percent *0.01)+" 】 中介金额【 "+ getPriceStr(input_buyer_buyhome_zhongjie_price) +" 】 ";
			String buyer_buyhome_downpayment_outtip="6.首付费率 【 "+getPercentStr(buyer_buyhome_downpayment_percent*0.01)+" 】 首付金额【 "+getPriceStr(buyer_buyhome_downpayment_price)+" 】 ";
			String buyer_all_tax_price_tip=   "_总税费额_____________________【 "+getPriceStr(buyer_all_tax_price)+" 】 ";

			String buyer_first_buyhome_price_tip=   "_总首付额_____________________【 "+getPriceStr(buyer_first_buyhome_price)+" 】 ";
			...
			logArr.add(head_tip_8);
			logArr.add(head_tip_9);
			logArr.add(input_tip);
			logArr.add(seller_home_isputong_flag_tip);
			logArr.add(seller_first_home_flag_tip);
			logArr.add(seller_home_fiveyear_flag_tip);
			logArr.add(buyer_first_home_flag_tip);
			logArr.add(home_area_tip);
			logArr.add(seller_buyhome_price_tip);
			logArr.add(buyer_buyhome_seller_sellhome_price_tip);
			logArr.add(buyer_buyhome_zhongjie_tip);
			logArr.add(buyer_buyhome_downpayment_tip);
			logArr.add(output_tip);
			logArr.add(buyer_buyhome_zengzhishui_tip);
			logArr.add(buyer_buyhome_fujiashui_tip);
			logArr.add(buyer_buyhome_geshui_tip);
			logArr.add(buyer_buyhome_qishui_tip);
			logArr.add(buyer_buyhome_zhongjie_outtip);
			logArr.add(buyer_buyhome_downpayment_outtip);
			...
			for (int i = 0; i < logArr.size(); i++) {
				String endPrintLine = "";
				if(logArr.get(i).startsWith("╔") || logArr.get(i).startsWith("╠") ||  logArr.get(i).startsWith("╚") ){

					endPrintLine = logArr.get(i)+"\n";
				}else{
					endPrintLine = "║"+logArr.get(i)+"║"+"\n";
				}
				sb.append(endPrintLine);
		...
		public  int getPaddingChineseLength(String oriStr) {
			int resultLength = 0;
		...
		public  int getChineseCount(String oriStr) {
			int count = 0;
...
|| (itemChar+"").equals("；")
		...
		String getPriceStr(double percentValue){
			...
			// 000,000,000.00  ->
			int mZeroAndSeperatorLenth = getZeroAndSeperatorLenth(priceStr);
			String padddingStr = getPaddingString(mZeroAndSeperatorLenth," ",true);

			String real_priceStr = priceStr.substring(mZeroAndSeperatorLenth);

			return padddingStr+real_priceStr;
		}




		String  getPaddingStrWithLength(int max_line_length ,String rawStr ,   String oneStr , boolean dirPre){
			String result = "";
			int distance = max_line_length - getPaddingChineseLength(rawStr)   ;
		...
		String  getPaddingString(int padinglength , String oneStr , boolean dirPre){
			String result = "";
			int distance = padinglength  ;
			for (int i = 0; i < distance; i++) {
				if(dirPre){
					result = oneStr+result;
				}else{
		...
		int getZeroAndSeperatorLenth(String mStr){
			int mZeroAndSeperatorLength = 0 ;

			int str_length = mStr.length();

			for (int i = 0; i < str_length; i++) {
				String curCharStr = mStr.substring(i,i+1);
				if("0".equals(curCharStr) || ",".equals(curCharStr)){
					mZeroAndSeperatorLength++;
				} else{

					return mZeroAndSeperatorLength;
				}
		...
		String getPaddingIntString(int index, int padinglength, String oneStr, boolean dirPre) {
			String result = "" + index;
...
//                double gongjijin_loan_month_count = 240;
	...
	class Add_EmptyCount_For_EveryOneLine_Rule_61 extends Basic_Rule {

		Add_EmptyCount_For_EveryOneLine_Rule_61(boolean mIsInputDirAsSearchPoint) {
			super(61);
			isInputDirAsSearchPoint = mIsInputDirAsSearchPoint;
		}

		Add_EmptyCount_For_EveryOneLine_Rule_61() {
			super(61, false);
		...
		ArrayList<File> applyOperationRule(ArrayList<File> curFileList, HashMap<String, ArrayList<File>> subFileTypeMap,
			...
			for (int i = 0; i < curInputFileList.size(); i++) {
				...
				writeContentToFile(I9_Temp_Text_File, fixedStrArr);
				NotePadOpenTargetFile(I9_Temp_Text_File.getAbsolutePath());
				System.out.println("rule_"+rule_index+" -> 把当前文件加入【行号_空格数】  File=" + fileItem.getAbsolutePath());
			}
		...
		@Override
		String simpleDesc() {
			return " 打印 每一行 【行号_起始空格数量】  保持原始空格(判断python文件的起始空格数量)";
		}
...
//    		newRulePrintMethodRaw.addAll(tipList);
	...
	class Add_Echo_Log_To_SH_BASH_Rule_59 extends Basic_Rule {
		...
		@Override
		String simpleDesc() {
			return "  把当前 bash  文件 和 sh 文件 py文件  空格插入echo  追踪 打印数据  然后 查看 执行 调用过程  ";
		}
...
//                Collections.copy(curTemplateList, templateStrList);
	...
	class ADB_Wireless_WIFI_Rule_22 extends Basic_Rule {
		...
		String adb_wireless_port_5str = "";  // adb 连接的 端口 5 位数  // 配对码的 端口 5位数

		String adb_pair_port_5str = "";   // 配对码的 端口 5位数

		String ipaddress = ""; // IP地址的最后一个值

		//        String IP_Addpress_Pre_1 = "192.168.0.";
		String IP_Addpress_Pre_1 = "192.168.0.";

		//        String IP_Addpress_Pre_2 = "10.106.20.";
		String IP_Addpress_Pre_2 = "";
...
//                ArrayList<String>  fixedStrArr =   add_system_out_Rule_21(fileItem);
				...
				ipaddress = getIPAddress(fileContent );

				System.out.println("6位配对码pair_code_6tr = " + pair_code_6tr);
				System.out.println("ip地址 = " + ipaddress);
				if (pair_code_6tr == null || ipaddress == null) { // 一：if
					...
					fixed_1_content = fixed_1_content.replace(ipaddress, " ");
					fixed_1_content = fixed_1_content.replace(":", " ");
					adb_wireless_port_5str = getDefineLengthDigital(fixed_1_content, 5);

					System.out.println("fixed_1_content = " + fixed_1_content);
					if (adb_wireless_port_5str == null) { // 一二：if
						fixedStrArr.add("[2]-> 无法搜索到 5位 无线端口码");
					} else {
						fixedStrArr.add("[2]-> 5位 无线端口码: " + adb_wireless_port_5str);

						String fixed_2_content = fixed_1_content.replace(adb_wireless_port_5str, " ");

						System.out.println("fixed_2_content = " + fixed_2_content);
						adb_pair_port_5str = getDefineLengthDigital(fixed_2_content, 5);

						if (adb_pair_port_5str == null) { // 一二三：if
							fixedStrArr.add("[3]-> 无法搜索到 5位 连接端口码");
						} else {
							fixedStrArr.add("[3]-> 5位 配对端口码: " + adb_pair_port_5str.replace("\n", ""));

							String fixed_3_content = fixed_2_content.replace(adb_pair_port_5str, "").trim();


							if (ipaddress == null) {
								fixedStrArr.add("[4]-> 无法搜索到 IP地址最后值");
							} else {
								fixedStrArr.add("[4]->IP地址: " + ipaddress);
								isAllSuccess = true;
				...
				if (isAllSuccess) {
					String IP_Address_1 = ipaddress;
					fixedStrArr.add("\n");
					fixedStrArr.add("═════════════════════IP地址:"   + "【" + IP_Address_1 + "】"
							+ "═════════════════════");
					String adbCommand_1_1 = calcul_Wireless_ADB_Command(IP_Address_1, adb_pair_port_5str,
							pair_code_6tr, adb_wireless_port_5str);
					String adbCommand_1_2 = calcul_Wireless_ADB_Command(IP_Address_1, adb_wireless_port_5str, pair_code_6tr,
							adb_pair_port_5str);
...
/*
				...
				System.out.println("rule_" + rule_index + " ->  执行打印无线连接命令 完毕!! "
						+ fileItem.getAbsolutePath());
			...
			for (int i = 0; i < strArr.length; i++) {
				...
				if (!isNumeric(strArr[i]) && strArr[i].trim().contains(".") ){
					...
					if (strArr[i].contains(":")){
						String realIP = strArr[i].substring(0,strArr[i].indexOf(":"));
						return realIP;
					}
					return strArr[i];
				}
