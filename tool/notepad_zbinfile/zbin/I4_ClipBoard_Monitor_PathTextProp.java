...
//固定1  zbin 的 字符串绝对路径
	...
	static File I4_Properties_File = new File(
	...
	static File I4_BEACH_MOVE_BAT = new File(System.getProperties().getProperty("user.home") + File.separator
			+ "Desktop" + File.separator + "zbin" + File.separator + "I4_beach_move" + ".bat");
	static File I4_BEACH_MOVE_SH = new File(System.getProperties().getProperty("user.home") + File.separator + "Desktop"
			+ File.separator + "zbin" + File.separator + "I4_beach_move" + ".sh");
...
//  固定属性列表 ------End
	...
	// ffmpeg -f concat -safe 0 -i C:\Users\zhuzj5\Desktop\zbin\I4_1_MergedRule.txt
	// -c copy C:\Users\zhuzj5\Desktop\output2.mp4
	// D:\software\ffmpeg\bin
	// D:\software\ffmpeg\bin\ffmpeg.exe -f concat -safe 0 -i
	// C:\Users\zhuzj5\Desktop\zbin\I4_1_MergedRule.txt -c copy
	...
	static {
		try {
			...
			I4_Properties_InputStream = new BufferedInputStream(
					new FileInputStream(I4_Properties_File.getAbsolutePath()));
			I4_Properties.load(I4_Properties_InputStream);
			Iterator<String> it = I4_Properties.stringPropertyNames().iterator();
			while (it.hasNext()) {
				String key = it.next();
				// System.out.println("key:" + key + " value: " +
				// I4_Properties.getProperty(key));
	...
	static void setProperity() {
		try {
			I4_Properties_OutputStream = new BufferedOutputStream(
					new FileOutputStream(I4_Properties_File.getAbsolutePath()));
...
//                         System.out.println("file found at path: " + file.toAbsolutePath());
	...
	static void showTip() {
		...
		System.out.println("系统的剪切板进行监听 对那些满足 条件的 video文件 保存 收藏路径到 Prop文件中 等待 bat 或者 sh 文件处理 ");
		System.out.println("PotPlayer播放器设置Tip:  F5(选项) >>  添加 (添加快捷按键) >>  空格 || * || = || - ");
		System.out.println(" 配置&语言&其他  >>  辅助处理  >>  将文件完整路径复制到剪切板");
		System.out.println(" " + Cur_Bat_Name);
		System.out.println("═══════════════════" + "使用方法列表 End " + "═══════════════════" + "\n");
		showMapPathInfo();
	}
...
//                    showMapPathInfoWhenBegin();
	...
	static void Operation(String clipText) {
		...
		System.out.println(diffTipStr + " time = " + getTimeStamp() + "   当前保存文件个数:" + curPropSIze + "  总文件大小:"
				+ getFileSize(propFileListSize));
...
//        fileCopy(originMP4File,newShellFile);
		...
		System.out.println(diffTipStr + "【索引: " + curPropSIze + "】" + "单文件大小● " + getJustFileSize(originMP4File) + "●"
				+ " 当前个数〓" + curPropSIze2 + "〓" + "  当前总大小▲ " + allSizeStr + "▲" + "═══" + clipText
				+ " End Success════════");
	}
	...
	static boolean isVideoFile(File file) {
		...
		if (fileName.endsWith(".mp4") || fileName.endsWith(".avi") || fileName.endsWith(".flv")
				|| fileName.endsWith(".rmvb") || fileName.endsWith(".mkv") || fileName.endsWith(".3gp")) {
			isVideoFlag = true;
